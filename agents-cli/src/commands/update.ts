  ...
  try {
    ...
    if (options.check) {
      if (versionInfo.needsUpdate) {
        ...
        console.log(chalk.gray(`  • Run ${chalk.cyan('agent-fabric update')} to update`));
      } else {
    ...
    if (!options.force) {
      const confirm = await p.confirm({
        message: `Update @agent-fabric/agents-cli from ${versionInfo.current} to ${versionInfo.latest}?`,
        initialValue: true,
      });
    ...
    // Execute update
    s.start(`Updating @agent-fabric/agents-cli to ${versionInfo.latest}...`);
    ...
    if (
      ...
      console.log(chalk.gray('  • sudo agent-fabric update'));
    }
