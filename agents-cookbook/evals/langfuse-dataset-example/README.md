...
2. Set up environment variables in a `.env` file:
   ...
   AGENT_FABRIC_AGENTS_RUN_API_KEY=your_api_key
   AGENT_FABRIC_AGENTS_RUN_API_URL=your_chat_api_base_url

   # Execution context (for dataset runner)
   AGENT_FABRIC_TENANT_ID=your_tenant_id
   AGENT_FABRIC_PROJECT_ID=your_project_id
   AGENT_FABRIC_AGENT_ID=your_agent_id
   ```
