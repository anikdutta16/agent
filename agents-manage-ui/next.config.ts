...
if (process.env.NODE_ENV !== 'production') {
  try {
    const { loadEnvironmentFiles } = require('@agent-fabric/agents-core');
...
// In the outer monorepo (agents-private) we want `@agent-fabric/agents-ui` edits to
...
//
// Two gates:
// - `existsSync`: after Copybara mirrors next.config.ts to `agent-fabric/agents`,
...
const agentsUiSourceAlias: Record<string, string> = useAgentsUiSource
  ? {
      '@agent-fabric/agents-ui': rel(path.join(agentsUiSourceDir, 'src/index.ts')),
      '@agent-fabric/agents-ui/types': rel(path.join(agentsUiSourceDir, 'src/types/index.ts')),
      '@agent-fabric/agents-ui/agent-fabric.css': rel(
        path.join(agentsUiSourceDir, 'src/styled/agent-fabric.css')
      ),
    }
  : {};

const nextConfig: NextConfig = {
  // Transpile @agent-fabric/agents-ui. In the outer monorepo the turbopack
  ...
  // harmless there.
  transpilePackages: ['@agent-fabric/agents-ui'],
  experimental: {
  ...
  reactCompiler: {
    ...
    // when the turbopack alias redirects @agent-fabric/agents-ui to its private
  ...
  turbopack: {
    ...
    rules: {
      ...
      // agents-ui's Shadow component does `import styles from '../agent-fabric.css?raw'`
      // (a Vite convention). When we alias @agent-fabric/agents-ui to source, this
