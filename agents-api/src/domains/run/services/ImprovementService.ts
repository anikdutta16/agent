...
import type { AgentsManageDatabaseClient } from '@agent-fabric/agents-core';
import {
  createApiError,
  createBranch,
  createConversation,
...
async function getCopilotConfig(): Promise<CopilotConfig> {
  ...
  copilotConfigPromise = (async () => {
    const appId = env.PUBLIC_AGENT_FABRIC_COPILOT_APP_ID;
    if (!appId) {
      throw createApiError({
        code: 'internal_server_error',
        message: 'PUBLIC_AGENT_FABRIC_COPILOT_APP_ID is required',
      });
    }
...
function signCopilotJwt(userId: string): string {
  const privateKeyB64 = env.AGENT_FABRIC_COPILOT_JWT_PRIVATE_KEY;
  const kid = env.AGENT_FABRIC_COPILOT_JWT_KID;

  if (!privateKeyB64 || !kid) {
    throw createApiError({
      code: 'internal_server_error',
      message: 'AGENT_FABRIC_COPILOT_JWT_PRIVATE_KEY and AGENT_FABRIC_COPILOT_JWT_KID are required',
    });
  }
...
export async function triggerImprovement(
  ...
  inProcessFetch('http://localhost/run/v1/chat/completions', {
    ...
    headers: {
      ...
      'x-agent-fabric-app-id': copilot.appId,
