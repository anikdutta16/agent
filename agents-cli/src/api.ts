...
abstract class BaseApiClient {
  ...
  protected checkTenantId(): string {
    if (!this.tenantId) {
      throw new Error('No tenant ID configured. Please run: agent-fabric init');
    }
  ...
  async getFullProject(projectId: string, ref?: string): Promise<FullProjectResponse['data']> {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        errorMessage += '  • Missing or invalid API key in agent-fabric.config.ts\n';
        errorMessage += '  • API key does not have access to this tenant/project\n';
        errorMessage +=
          '  • For local development, ensure AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET is set\n';
        if (errorText) {
  ...
  async chatCompletion(
    ...
    const response = await this.authenticatedFetch(`${this.apiUrl}/run/v1/chat/completions`, {
      ...
      headers: {
        ...
        'x-agent-fabric-tenant-id': this.tenantId || 'test-tenant-id',
        'x-agent-fabric-project-id': this.projectId,
