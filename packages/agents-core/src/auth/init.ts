 ...
 * - AGENT_FABRIC_AGENTS_RUN_DATABASE_URL: PostgreSQL connection string
 * - TENANT_ID: Organization/tenant ID (defaults to 'default') - this becomes the org ID
 * - AGENT_FABRIC_AGENTS_MANAGE_UI_USERNAME: Admin email address
 * - AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD: Admin password (min 15 chars, see password policy)
 * - BETTER_AUTH_SECRET: Secret for Better Auth
 *
 * Optional environment variables:
 * - AGENT_FABRIC_AGENTS_API_URL: API URL for Better Auth (defaults to http://localhost:3002)
 * - AGENT_FABRIC_AUTH_INIT_FORCE_PASSWORD_RESET: When 'true' and the admin user
 *   already exists, re-sync the user's credential-account password from
 *   AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD. Intended for ephemeral CI envs where
...
async function init() {
  ...
  const username = process.env.AGENT_FABRIC_AGENTS_MANAGE_UI_USERNAME;
  const password = process.env.AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD;
  const authSecret = process.env.BETTER_AUTH_SECRET;

  if (!username || !password) {
    console.error(
      '❌ AGENT_FABRIC_AGENTS_MANAGE_UI_USERNAME and AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD are required'
    );
  ...
  if (passwordViolations.length > 0) {
    console.error('❌ AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD does not meet the password policy:');
  ...
  const apiUrl = process.env.AGENT_FABRIC_AGENTS_API_URL || 'http://localhost:3002';
  ...
  if (user) {
    if (process.env.AGENT_FABRIC_AUTH_INIT_FORCE_PASSWORD_RESET === 'true') {
  ...
  const playgroundAppId = process.env.AGENT_FABRIC_PLAYGROUND_APP_ID || 'app_playground';
  const tempJwtPublicKeyB64 = process.env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY;

  console.log(`\n🎮 Checking/creating playground app: ${playgroundAppId}`);

  // Derive allowed domains from AGENT_FABRIC_AGENTS_MANAGE_UI_URL
  let allowedDomains: string[] = ['localhost', '127.0.0.1'];
  const manageUiUrl = process.env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL;
  if (manageUiUrl) {
    try {
      ...
      console.log(
        `   ⚠️  Invalid AGENT_FABRIC_AGENTS_MANAGE_UI_URL: ${manageUiUrl}, using localhost defaults`
      );
    }
  }
  ...
  if (existingApp) {
    ...
    if (tempJwtPublicKeyB64) {
      ...
      console.log(
        '   ⚠️  AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY not set — playground app created without auth keys'
      );
    }
