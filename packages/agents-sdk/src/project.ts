...
import { convertZodToJsonSchema, getLogger, isZodSchema } from '@agent-fabric/agents-core';
...
export class Project implements ProjectInterface {
  ...
  constructor(config: ProjectConfig) {
    ...
    this.tenantId = process.env.AGENT_FABRIC_TENANT_ID || 'default';
    this.baseURL = process.env.AGENT_FABRIC_API_URL || 'http://localhost:3002';
   ...
   * This is used by the CLI to inject configuration from agent-fabric.config.ts
   */
  setConfig(
    tenantId: string,
    apiUrl: string,
