...
Enables enterprise teams to interact with Agent Fabric AI agents directly from Slack via slash commands and @mentions.

## Quick Start

1. Start the API server: `pnpm dev` from monorepo root
2. Install the Slack app to a workspace via `/work-apps/slack/install`
3. Use `/agent-fabric help` in Slack to see available commands
...
| Context | Priority |
|---------|----------|
| `/agent-fabric` commands | Channel default > Workspace default |
| `@Agent Fabric` mentions | Channel default > Workspace default |

### Slash Commands

| Command | Description |
|---------|-------------|
| `/agent-fabric` | Open agent picker modal |
| `/agent-fabric help` | Show available commands |
| `/agent-fabric link` | Link Slack account to Agent Fabric |
| `/agent-fabric status` | Check account and agent status |
| `/agent-fabric [message]` | Send a message to the default agent |
...
│                              SLACK WORKSPACE                                │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐         │
│  │  /agent-fabric cmd    │    │  @Agent Fabric        │    │  #channel       │         │
│  └────────┬────────┘    └────────┬────────┘    └────────┬────────┘         │
└───────────│──────────────────────│──────────────────────│───────────────────┘
            │                      │                      │
            ▼                      ▼                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                    AGENT FABRIC API (/work-apps/slack/*)                   │
...
1. **Create a dev Slack app**
   ...
   - Modify it: set `settings.socket_mode_enabled` to `true`, remove all `request_url` and `url` fields, rename the app (e.g., "Agent Fabric Dev - YourName")
...
- Ensure `@slack/socket-mode` is installed: `pnpm --filter @agent-fabric/agents-work-apps add -D @slack/socket-mode`
...
- Run: `pnpm --filter @agent-fabric/agents-work-apps add -D @slack/socket-mode`
...
### "User not linked"
Run `/agent-fabric link` and click the link to complete the linking flow.
...
### "Link token expired"
JWT link tokens expire after 10 minutes. Run `/agent-fabric link` again.
