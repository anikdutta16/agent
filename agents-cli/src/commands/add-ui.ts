...
export async function addUiCommand(options: AddUiOptions): Promise<void> {
  ...
  if (!configPath) {
    console.error(
      chalk.red(
        'No Agent Fabric config found. Run from a project directory with agent-fabric.config.ts or pass --config <path>.'
      )
    );
    process.exit(1);
  }
  ...
  if (!projectId) {
    console.error(
      chalk.red('Project ID not found in config. Set projectId in your agent-fabric.config.ts.')
    );
    process.exit(1);
  }
  ...
  if (!config.agentsApiKey) {
    console.error(
      chalk.red(
        'Not authenticated. Run "agent-fabric login" or set agentsApi.apiKey in your config.'
      )
    );
    process.exit(1);
  }
  ...
  if (options.list) {
    ...
    s.stop();
    console.log(
      chalk.cyan('\nAvailable UI components (use id with agent-fabric add --ui <id>):\n')
    );
