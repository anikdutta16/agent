import type { SlackLinkIntent } from '@agent-fabric/agents-core';
import { getInProcessFetch, signSlackUserToken } from '@agent-fabric/agents-core';
import { env } from '../../env';
...
export async function resumeSmartLinkIntent(params: ResumeSmartLinkIntentParams): Promise<void> {
  const { intent, teamId, slackUserId, agentFabricUserId, tenantId, slackEnterpriseId } = params;
...
async function resumeMention(
  ...
  if (!replyThreadTs) {
    ...
    await postErrorToChannel(
      ...
      slackUserId,
      undefined,
      "We couldn't resume your question due to a technical issue. Please try mentioning @Agent Fabric again."
    );
    return;
  }
...
async function findAgentByIdentifierViaApi(
  ...
  const apiBaseUrl = env.AGENT_FABRIC_AGENTS_API_URL || 'http://localhost:3002';
  ...
  try {
    response = await getInProcessFetch()(`${apiBaseUrl}/run/api/chat`, {
      ...
      headers: {
        ...
        'x-agent-fabric-project-id': projectId,
        'x-agent-fabric-agent-id': agentId,
        'x-agent-fabric-invocation-type': 'slack',
        'x-agent-fabric-invocation-entry-point': 'smart_link_resume',
      },
      body: JSON.stringify({
