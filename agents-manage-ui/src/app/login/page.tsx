...
import type { MethodOption, OrgAuthInfo } from '@agent-fabric/agents-core/auth/auth-types';
import { authLookupResponseSchema } from '@agent-fabric/agents-core/auth/auth-types';
import { AlertCircleIcon, ArrowLeft, Globe, Loader2, Mail } from 'lucide-react';
import Link from 'next/link';
import { useRouter, useSearchParams } from 'next/navigation';
import { Suspense, useEffect, useState } from 'react';
import { AgentFabricIcon } from '@/components/icons/agent-fabric';
...
function LoginForm() {
  ...
  const { PUBLIC_IS_SMTP_CONFIGURED, PUBLIC_AGENT_FABRIC_AGENTS_API_URL } = useRuntimeConfig();
  ...
  const oauthAuthorizeUrl = isOAuthFlow
    ? `${PUBLIC_AGENT_FABRIC_AGENTS_API_URL}/api/auth/oauth2/authorize?${searchParams.toString()}`
    : null;
  ...
  const handleEmailContinue = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${PUBLIC_AGENT_FABRIC_AGENTS_API_URL}/manage/api/auth-lookup?email=${encodeURIComponent(email)}`
      );
