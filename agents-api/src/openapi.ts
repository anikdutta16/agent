import { SESSION_COOKIE_NAME } from '@agent-fabric/agents-core';
...
export function setupOpenAPIRoutes<E extends Env = Env>(app: OpenAPIHono<E>) {
  ...
  app.get('/openapi.json', (c: Context) => {
    try {
      ...
      const document = app.getOpenAPIDocument({
        openapi: '3.1.0',
        info: {
          title: 'Agent Fabric API',
          version: '1.0.0',
          description: 'REST API for the Agent Fabric.',
        },
        servers: [
          {
            url: serverUrl,
  ...
  app.get(
    ...
    swaggerUI({
      url: '/openapi.json',
      title: 'Agent Fabric API Documentation',
    })
  );
}
