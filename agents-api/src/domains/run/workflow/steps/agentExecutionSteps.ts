import type { FullExecutionContext, McpTool, Part, ResolvedRef } from '@agent-fabric/agents-core';
import { SPAN_NAMES, TRANSFER_TOOL_PREFIX } from '@agent-fabric/agents-core';
...
async function buildAgentForStep(params: {
  ...
  const {
    ...
    getMcpToolById,
    withRef,
  } = await import('@agent-fabric/agents-core');
  ...
  const apiBaseUrl = env.AGENT_FABRIC_AGENTS_API_URL;
  ...
  const executionContext: FullExecutionContext = {
    ...
    apiKey: env.AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET || '',
...
export async function markWorkflowRunningStep(params: {
  ...
  const { createWorkflowExecution } = await import('@agent-fabric/agents-core');
...
export async function callLlmStep(params: CallLlmStepParams): Promise<CallLlmResult> {
  ...
  return runWithLogContext(
    ...
    async () => {
      ...
      const { createMessage, generateId, updateTask } = await import('@agent-fabric/agents-core');
      const { default: runDbClient } = await import('../../../../data/db/runDbClient');
          ...
          return await tracer.startActiveSpan(
            ...
            async (span) => {
              try {
                ...
                const { updateTask } = await import('@agent-fabric/agents-core');
                const { default: runDbClient } = await import('../../../../data/db/runDbClient');
                try {
                  await updateTask(runDbClient)({
                    taskId: params.taskId,
...
export async function markWorkflowSuspendedStep(params: {
  ...
  const { updateWorkflowExecutionStatus } = await import('@agent-fabric/agents-core');
  const { default: runDbClient } = await import('../../../../data/db/runDbClient');
