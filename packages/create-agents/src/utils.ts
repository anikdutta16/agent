...
import { ANTHROPIC_MODELS, GOOGLE_MODELS, OPENAI_MODELS } from '@agent-fabric/agents-core';
...
const agentsTemplateRepo = 'https://github.com/agent-fabric/agents/create-agents-template';

const projectTemplateRepo =
  'https://github.com/agent-fabric/agents/agents-cookbook/template-projects';
  ...
  for (const pkgPath of packageJsonPaths) {
    ...
    for (const depType of ['dependencies', 'devDependencies'] as const) {
      ...
      for (const name of Object.keys(deps)) {
        if (name.startsWith('@agent-fabric/') && DIVERGENT_PACKAGES.has(name)) {
          allDeps.add(name);
        }
      }
    }
  ...
  await Promise.all(
    pkgDataList.map(async ({ pkgPath, pkg }) => {
      for (const depType of ['dependencies', 'devDependencies'] as const) {
        ...
        if (!deps) continue;
        for (const name of Object.keys(deps)) {
          if (!name.startsWith('@agent-fabric/')) continue;
...
export const createAgents = async (
  args: {
    ...
    skipAgentFabricCli?: boolean;
    skipAgentFabricMcp?: boolean;
    skipInstall?: boolean;
  ...
  try {
    ...
    s.message('Creating agent-fabric.config.ts...');
    await createAgentFabricConfig(config);
    ...
    if (!skipAgentFabricCli) {
      let isGloballyInstalled = false;

      try {
        const { stdout } = await execAsync('pnpm list -g @agent-fabric/agents-cli --json');
        const result = JSON.parse(stdout);
        isGloballyInstalled = result?.[0]?.dependencies?.['@agent-fabric/agents-cli'] !== undefined;
      } catch (_error) {
        try {
          await execAsync('npm list -g @agent-fabric/agents-cli');
      ...
      if (!isGloballyInstalled) {
        const installAgentFabricCLIResponse = await p.confirm({
          message: 'Would you like to install the Agent Fabric CLI globally?',
        });
        if (!p.isCancel(installAgentFabricCLIResponse) && installAgentFabricCLIResponse) {
          await installAgentFabricCLIGlobally();
        }
      }
    }

    if (!skipAgentFabricMcp) {
      await addAgentFabricMcp();
    }
    ...
    p.note(
      `${color.yellow('1. Start:')}\n` +
        ...
        `   • Use 'agent-fabric push' to apply`,
      'Next steps 🚀'
    );
  } catch (error) {
    s.stop();
...
async function createEnvironmentFiles(config: FileConfig) {
  ...
  const injections: Record<string, string> = {
    ...
    AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET: bypassSecret,
  };
...
async function createAgentFabricConfig(config: FileConfig) {
  const agentFabricConfig = `import { defineConfig } from '@agent-fabric/agents-cli/config';
...
export default config;`;
  await fs.writeFile(`src/agent-fabric.config.ts`, agentFabricConfig);

  if (config.customProject) {
    const customIndexContent = `import { project } from '@agent-fabric/agents-sdk';
...
async function installAgentFabricCLIGlobally() {
  const s = p.spinner();
  s.start('Installing Agent Fabric CLI globally with pnpm...');

  try {
    await execAsync('pnpm add -g @agent-fabric/agents-cli');
    s.stop('✓ Agent Fabric CLI installed successfully with pnpm');
    return;
  } catch (_pnpmError) {
    s.message('pnpm failed, trying npm...');

    try {
      await execAsync('npm install -g @agent-fabric/agents-cli');
      s.stop('✓ Agent Fabric CLI installed successfully with npm');
      return;
    } catch (_npmError) {
      s.stop('⚠️  Could not automatically install Agent Fabric CLI globally');
      console.warn('You can install it manually later by running:');
      console.warn('  npm install -g @agent-fabric/agents-cli');
      console.warn('  or');
      console.warn('  pnpm add -g @agent-fabric/agents-cli\n');
    }
  }
}
...
async function initializeGit() {
  try {
    ...
    await execAsync('git commit -m "Initial commit from agent-fabric/create-agents"');
  } catch (error) {
    console.error(
...
export async function addAgentFabricMcp() {
  const editorChoice = await p.select({
    message: 'Give your IDE access to Agent Fabric docs and types? (Adds Agent Fabric MCP)',
    options: [
  ...
  try {
    const mcpConfig = {
      mcpServers: {
        'agent-fabric': {
          type: 'mcp',
          url: 'https://agents.localhost/mcp',
        },
      },
    };
    ...
    switch (editorChoice) {
      case 'cursor-project': {
        s.start('Adding Agent Fabric MCP to Cursor (project)...');
        ...
        s.stop(`${color.green('✓')} Agent Fabric MCP added to .cursor/mcp.json`);
        break;
      }

      case 'cursor-global': {
        s.start('Adding Agent Fabric MCP to Cursor (global)...');
        ...
        await fs.writeJson(configPath, mergedConfig, { spaces: 2 });
        s.stop(`${color.green('✓')} Agent Fabric MCP added to global Cursor settings`);
        break;
      }

      case 'windsurf': {
        s.start('Adding Agent Fabric MCP to Windsurf...');
        ...
        await fs.writeJson(configPath, mergedConfig, { spaces: 2 });
        s.stop(`${color.green('✓')} Agent Fabric MCP added to Windsurf settings`);
        break;
      }

      case 'vscode': {
        s.start('Adding Agent Fabric MCP to VSCode...');
        ...
        await fs.writeJson(configPath, mergedConfig, { spaces: 2 });
        s.stop(
          `${color.green('✓')} Agent Fabric MCP added to VSCode settings\n\n${color.yellow('Next steps:')}\n` +
