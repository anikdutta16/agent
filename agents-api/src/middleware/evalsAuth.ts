 ...
 * First checks if token matches AGENT_FABRIC_AGENTS_EVAL_API_BYPASS_SECRET,
 */
export const evalApiKeyAuth = () => {
  const mw = createMiddleware<{
    ...
    if (env.AGENT_FABRIC_AGENTS_EVAL_API_BYPASS_SECRET) {
      // Check for Bearer token
      if (!authHeader || !authHeader.startsWith('Bearer ')) {
      ...
      const apiKey = authHeader.substring(7); // Remove 'Bearer ' prefix
      const tokenMatches = apiKey === env.AGENT_FABRIC_AGENTS_EVAL_API_BYPASS_SECRET;
      if (tokenMatches) {
