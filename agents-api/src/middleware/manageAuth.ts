...
async function tryOAuthUserAuth(token: string): Promise<OAuthUserJwtAttempt> {
  ...
  const tenantId =
    typeof payload['http://localhost/tenantId'] === 'string'
      ...
      : undefined;
  const userEmail =
    typeof payload['http://localhost/email'] === 'string'
      ? (payload['http://localhost/email'] as string)
      : undefined;
 ...
 * 1. Bypass secret (AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET)
...
export const manageBearerAuth = () =>
  createMiddleware<{
    ...
    if (
      ...
      token === env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET
    ) {
    ...
    if (env.COPILOT_OAUTH_CLIENT_ID && token.split('.').length === 3) {
      try {
        ...
        const tenantId = payload['http://localhost/tenantId'] as string | undefined;
        const email = payload['http://localhost/email'] as string | undefined;
        if (sub) {
    ...
    if (isSlackUserToken(token)) {
      ...
      logger.info(
        {
          agentFabricUserId: result.payload.sub,
    ...
    if (isInternalServiceToken(token)) {
      ...
      c.set('userEmail', `${result.payload.sub}@internal.agent-fabric`);
