...
export function createAgentsRunDatabaseClient(
  ...
  const connectionString = config?.connectionString || env.AGENT_FABRIC_AGENTS_RUN_DATABASE_URL;
  ...
  if (!connectionString) {
    throw new Error(
      'AGENT_FABRIC_AGENTS_RUN_DATABASE_URL environment variable is required. Please set it to your PostgreSQL connection string.'
    );
  }
