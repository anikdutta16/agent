...
const OSS_SYNC_BOT_NAME = 'agent-fabric-oss-sync[bot]';
const OSS_SYNC_BOT_EMAIL = '274976938+agent-fabric-oss-sync[bot]@users.noreply.github.com';
...
async function githubRequest({
  ...
  const response = await fetch(`https://api.github.com${requestPath}`, {
    ...
    headers: {
      ...
      'User-Agent': 'agent-fabric-public-pr-bridge',
...
Mirror public PR [#${publicPr.number}](${publicPr.html_url}) from \`${publicPr.base.repo.full_name}\` into \`agent-fabric/agents-private\` for internal review and merge.
...
async function syncPublicPr() {
  ...
  if (!metadataOnlyAction) {
    ...
    //      drifted from `agent-fabric/<repo>/main`. Without this, every conflicting
    ...
    try {
      ...
      // intermediate blob SHAs that only exist in agent-fabric/agents' object
