...
function hasExternalDatabases(): boolean {
  const manageUrl = process.env.AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL;
  const runUrl = process.env.AGENT_FABRIC_AGENTS_RUN_DATABASE_URL;
...
async function generateSecrets() {
  ...
  const jwtKeyHasValue =
    envContent.includes('AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY=') &&
    !envContent.includes('# AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY=') &&
    !!envContent.match(/AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY=(.+)/)?.[1]?.trim();
  ...
  if (!jwtKeyHasValue) {
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (
          lines[i].startsWith('# AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY=') ||
          lines[i].startsWith('AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY=')
        ) {
          lines[i] = `AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY=${privateKeyBase64}`;
          process.env.AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY = privateKeyBase64;
          privateKeyFound = true;
        }
        if (
          lines[i].startsWith('# AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY=') ||
          lines[i].startsWith('AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY=')
        ) {
          lines[i] = `AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY=${publicKeyBase64}`;
          process.env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY = publicKeyBase64;
          publicKeyFound = true;
        }
      }

      if (!privateKeyFound) {
        lines.push(`AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY=${privateKeyBase64}`);
        process.env.AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY = privateKeyBase64;
      }
      if (!publicKeyFound) {
        lines.push(`AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY=${publicKeyBase64}`);
        process.env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY = publicKeyBase64;
      }
  ...
  const secretDefs: Array<{
    ...
    {
      varName: 'AGENT_FABRIC_AGENTS_JWT_SIGNING_SECRET',
      placeholders: [],
    ...
    {
      varName: 'AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD',
      placeholders: ['change-me-Pass!@1234'],
      generate: () => generateCompliantPassword(),
    },
    {
      varName: 'AGENT_FABRIC_ANON_JWT_SECRET',
      placeholders: [],
      generate: () => randomBytes(32).toString('hex'),
    },
  ];
...
async function initAuth(authInitCommand: string) {
  const hasCredentials =
    process.env.AGENT_FABRIC_AGENTS_MANAGE_UI_USERNAME &&
    process.env.AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD &&
  ...
  if (hasCredentials) {
    ...
    logInfo('  AGENT_FABRIC_AGENTS_MANAGE_UI_USERNAME=admin@example.com');
    logInfo('  AGENT_FABRIC_AGENTS_MANAGE_UI_PASSWORD=your-password');
...
async function pushProject(pushConfig: SetupPushConfig) {
  ...
  // (supports cloud users who have run `agent-fabric login`).
  const pushEnv = pushConfig.apiKey
    ? {
        ...process.env,
        AGENT_FABRIC_CI: 'true',
        AGENT_FABRIC_API_KEY: pushConfig.apiKey,
        AGENT_FABRIC_TENANT_ID: process.env.AGENT_FABRIC_TENANT_ID || 'default',
        ...(pushConfig.apiUrl ? { AGENT_FABRIC_AGENTS_API_URL: pushConfig.apiUrl } : {}),
      }
    : { ...process.env };

  try {
    const pushCmd = `pnpm exec agent-fabric push --project ${pushConfig.projectPath} --config ${pushConfig.configPath}`;
    ...
    logInfo(
      `  pnpm exec agent-fabric push --project ${pushConfig.projectPath} --config ${pushConfig.configPath}`
    );
    return false;
  }
}
...
export async function runSetup(config: SetupConfig) {
  ...
  if (!config.isCloud) {
    ...
    if (!process.env.AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL)
      missing.push('AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL');
    if (!process.env.AGENT_FABRIC_AGENTS_RUN_DATABASE_URL)
      missing.push('AGENT_FABRIC_AGENTS_RUN_DATABASE_URL');
    if (missing.length > 0) {
    ...
    try {
      ...
      for (const push of pushConfigs) {
        const resolvedPush: SetupPushConfig = {
          ...push,
          apiKey: push.apiKey || process.env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET,
        };
