...
export async function pushCommand(options: PushOptions): Promise<void> {
  ...
  try {
    ...
    if (options.env) {
      ...
      process.env.AGENT_FABRIC_ENV = options.env;
    ...
    const originalTenantId = process.env.AGENT_FABRIC_TENANT_ID;
    const originalApiUrl = process.env.AGENT_FABRIC_API_URL;

    process.env.AGENT_FABRIC_TENANT_ID = config.tenantId;
    process.env.AGENT_FABRIC_API_URL = config.agentsApiUrl;
    ...
    if (originalTenantId !== undefined) {
      process.env.AGENT_FABRIC_TENANT_ID = originalTenantId;
    } else {
      delete process.env.AGENT_FABRIC_TENANT_ID;
    }
    if (originalApiUrl !== undefined) {
      process.env.AGENT_FABRIC_API_URL = originalApiUrl;
    } else {
      delete process.env.AGENT_FABRIC_API_URL;
    }
      ...
      if (lastPulledHash) {
        ...
        try {
          ...
          if (preview.hasConflicts) {
            ...
            console.log(
              chalk.yellow(
                '\nRun `agent-fabric pull` to resolve conflicts before pushing, or use `agent-fabric push --force` to overwrite remote.'
              )
            );
            process.exit(1);
          }
          ...
          console.log(
            chalk.yellow(
              `\n⚠ Conflict detection failed. Run \`agent-fabric pull\` first if you want to ensure no conflicts exist.`
            )
          );
        }
      }
    ...
    // Provide next steps
    console.log(chalk.green('\n✨ Next steps:'));
    console.log(chalk.gray(`  • View all agents: agent-fabric list-agent`));
...
async function pushAllProjects(options: PushOptions): Promise<void> {
  ...
  if (projectDirs.length === 0) {
    const configPattern = options.tag
      ? `${options.tag}.__agent-fabric.config.ts__`
      ...
      if (lastPulledHash) {
        ...
        if (preview.hasConflicts) {
          return {
            ...
            success: false,
            error: `${preview.conflicts.length} conflict(s) detected — run \`agent-fabric pull\` to resolve or use \`--force\``,
          };
        }
      }
    }
