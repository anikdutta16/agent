...
import { agentFabricFailedResponseHandler } from './agent-fabric-error';
import { convertToAgentFabricChatMessages } from './convert-to-agent-fabric-messages';
import { getResponseMetadata } from './get-response-metadata';
import { mapAgentFabricFinishReason } from './map-agent-fabric-finish-reason';

interface AgentFabricChatConfig {
  provider: string;
  baseURL: string;
...
export class AgentFabricChatLanguageModel implements LanguageModelV2 {
  ...
  readonly options: AgentFabricChatOptions;
  readonly config: AgentFabricChatConfig;

  get provider(): string {
    return this.config.provider;
  }

  constructor(options: AgentFabricChatOptions, config: AgentFabricChatConfig) {
    this.modelId = 'agent-fabric-agent';
  ...
  private getArgs(options: LanguageModelV2CallOptions) {
    ...
    return {
      args: {
        messages: convertToAgentFabricChatMessages(options.prompt),
  ...
  async doGenerate(options: LanguageModelV2CallOptions) {
    ...
    const response = await postJsonToApi({
      ...
      failedResponseHandler: agentFabricFailedResponseHandler,
      successfulResponseHandler: createJsonResponseHandler(agentFabricChatCompletionSchema),
    ...
    return {
      content,
      finishReason: mapAgentFabricFinishReason(choice.finish_reason as AgentFabricFinishReason),
      usage: {
            ...
            switch (event.type) {
              ...
              case 'finish':
                ...
                if (event.finishReason) {
                  finishReason = mapAgentFabricFinishReason(
                    event.finishReason as AgentFabricFinishReason
                  );
                }
                if (event.usage) {
                  usage = {
...
const agentFabricChatCompletionSchema = z.object({
  id: z.string(),
