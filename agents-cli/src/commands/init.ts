...
/**
 * Full onboarding wizard for Agent Fabric Cloud customers
 */
async function cloudInitCommand(options?: InitOptions): Promise<void> {
  console.log();
  console.log(chalk.bold('Welcome to Agent Fabric!'));
  console.log();
  ...
  if (keychainAvailable) {
    ...
    try {
      const existingCreds = await loadCredentials('agent-fabric-cloud');
  ...
  if (!isAuthenticated) {
    ...
    const newCreds = await loadCredentials('agent-fabric-cloud');
  ...
  try {
    const response = await fetch('http://localhost:3002/manage/api/cli/me', {
      headers: {
  ...
  try {
    const response = await fetch(
      `http://localhost:3002/manage/tenants/${selectedTenantId}/projects?limit=100`,
      {
        headers: {
          Authorization: `Bearer ${credentials?.accessToken}`,
        },
  ...
  if (options?.path) {
    ...
    const suggestedPath = './agent-fabric';
  ...
  if (projects.length === 0) {
    ...
    writeFileSync(join(templateDir, 'agent-fabric.config.ts'), configContent);
    ...
    console.log(chalk.gray(`        ├── agent-fabric.config.ts`));
    ...
    for (const project of projects) {
      ...
      writeFileSync(join(projectDir, 'agent-fabric.config.ts'), configContent);
  ...
  if (projects.length > 0) {
    console.log(chalk.gray('  3. Run: agent-fabric pull --all'));
  } else {
    console.log(chalk.gray('  3. Define your agent in index.ts'));
    console.log(chalk.gray('  4. Run: agent-fabric push'));
  }
...
async function localInitCommand(options?: InitOptions): Promise<void> {
  ...
  if (options?.path) {
    ...
    if (options.path.endsWith('.ts') || options.path.endsWith('.js')) {
      ...
      configPath = join(resolvedPath, 'agent-fabric.config.ts');
    }
  } else {
    const projectRoot = findProjectRoot(process.cwd());
    const suggestedPath = join(projectRoot, 'agent-fabric.config.ts');
    ...
    if (options?.interactive === false) {
      ...
      configPath =
        confirmedPath.endsWith('.ts') || confirmedPath.endsWith('.js')
          ? resolvedPath
          : join(resolvedPath, 'agent-fabric.config.ts');
    }
  }
  ...
  const configContent = `import { defineConfig } from '@agent-fabric/agents-cli/config';
...
`;
  ...
  try {
    ...
    console.log(chalk.gray('\nYou can now use the Agent Fabric CLI commands.'));
...
function generateConfigFile(tenantId: string, projectId: string): string {
  return `import { defineConfig } from '@agent-fabric/agents-cli/config';
...
function generateIndexFile(projectId: string): string {
  return `import { project } from '@agent-fabric/agents-sdk';

// This file was auto-generated by 'agent-fabric init'
// Run 'agent-fabric pull' to sync with your remote project
