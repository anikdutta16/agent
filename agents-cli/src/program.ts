...
export function createProgram(): Command {
  ...
  program.name('agent-fabric').description('CLI tool for Agent Fabric').version(PACKAGE_VERSION);
  ...
  program
    .command('init [path]')
    .description('Initialize a new Agent Fabric project (runs cloud onboarding wizard by default)')
  ...
  const configCommand = program.command('config').description('Manage Agent Fabric configuration');
  ...
  program
    ...
    .option(
      '--tag <tag>',
      'Use tagged config file (e.g., --tag prod loads prod.__agent-fabric.config.ts__)'
    )
    .option('--quiet', 'Suppress profile/config logging')
  ...
  program
    .command('dev')
    .description('Start the Agent Fabric dashboard server')
    ...
    .option('--output-dir <dir>', 'Output directory for build files', './agent-fabric-dev')
  ...
  program
    .command('update')
    .description('Update @agent-fabric/agents-cli to the latest version')
  ...
  program
    .command('login')
    .description('Authenticate with Agent Fabric Cloud')
    .option('--profile <name>', 'Profile to authenticate (defaults to active profile)')
    .action(loginCommand);

  program
    .command('logout')
    .description('Log out of Agent Fabric Cloud')
