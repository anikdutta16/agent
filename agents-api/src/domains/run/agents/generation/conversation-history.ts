import type { FilePart } from '@agent-fabric/agents-core';
import {
...
import {
  ...
  normalizeMimeType,
} from '@agent-fabric/agents-core/constants/allowed-file-formats';
import { isTextDocumentMimeType } from '@agent-fabric/agents-core/text-attachments';
...
import {
  AGENT_FABRIC_CACHE_BOUNDARY_PROP,
  SYSTEM_CACHE_BOUNDARY_SENTINEL,
} from './caching-actuator';
  ...
  if (historySegments.length > 0) {
    ...
    messages.push({
      ...
      content: historySegments.map((text, i) => ({
        type: 'text',
        text,
        ...(i === lastMessageSegmentIndex ? { [AGENT_FABRIC_CACHE_BOUNDARY_PROP]: 'history' } : {}),
      })),
    });
