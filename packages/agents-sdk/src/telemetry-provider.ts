 ...
 * import { AgentFabricTelemetryProvider, createConsoleTelemetryProvider } from '@agent-fabric/agents-sdk'
 *
...
export class ConsoleTelemetryProvider implements TelemetryProvider {
  ...
  constructor(serviceName = 'agent-fabric-agent') {
 ...
 */
export function createNoOpTelemetryProvider(): AgentFabricTelemetryProvider {
  return new AgentFabricTelemetryProvider({ enabled: false });
}

/**
 * Create a console-based telemetry provider for development
 */
export function createConsoleTelemetryProvider(serviceName?: string): AgentFabricTelemetryProvider {
  return new AgentFabricTelemetryProvider({
    enabled: true,
    serviceName,
...
): Promise<AgentFabricTelemetryProvider> {
  // Check if OpenTelemetry is available
