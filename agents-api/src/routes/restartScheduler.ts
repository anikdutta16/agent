...
restartWorkflowHandler.openapi(
  createProtectedRoute({
    ...
    description:
      'Called by CI after deploy to restart the scheduler workflow on the latest deployment. Auth via AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET.',
    permission: noAuth(),
    responses: {
  ...
  async (c) => {
    ...
    if (!constantTimeEqual(token, env.AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET)) {
      return c.json({ error: 'Unauthorized' }, 401);
    }
