import {
  AgentFabricAgentsCore,
  createConsoleLogger,
  createMCPServer,
  HeaderForwardingHook,
  SDKHooks,
  type SDKOptions,
} from '@agent-fabric/agents-mcp';
...
import { AGENT_FABRIC_MCP_ALLOWED_TOOLS } from '../mcpAllowedTools';
import { bindTenantId } from '../mcpGlobalParams';
import { AGENT_FABRIC_MCP_INSTRUCTIONS, setServerInstructions } from '../mcpServerInstructions';
...
app.all('/', async (c) => {
  ...
  if (bearer && bearer.split('.').length === 3) {
    try {
      ...
      const tenantClaim = payload['http://localhost/tenantId'];
  ...
  const createSDKWithHeaders = () => {
    ...
    return new AgentFabricAgentsCore({
      serverURL: env.AGENT_FABRIC_AGENTS_API_URL,
      hooks,
  ...
  const mcpServer = createMCPServer({
    ...
    allowedTools: [...AGENT_FABRIC_MCP_ALLOWED_TOOLS],
  });
  ...
  setServerInstructions(mcpServer, AGENT_FABRIC_MCP_INSTRUCTIONS);
  if (sessionTenantId) {
