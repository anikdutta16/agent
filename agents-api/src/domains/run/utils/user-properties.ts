...
// `x-agent-fabric-user-properties` and `x-agent-fabric-properties` are **widget-internal
// transport headers**, not part of the public chat API contract. They exist
// because the @agent-fabric/agents-ui-cloud widget injects userProperties at the
...
// `x-agent-fabric-*` headers are not part of that surface.
//
...
export function parseAgentFabricJsonHeader(
  ...
  try {
    ...
    opts?.logger?.warn(
      ...
      'Agent Fabric JSON header parsed but is not a plain object; ignoring'
    );
    return undefined;
  } catch (err) {
    opts?.logger?.warn(
      ...
      'Failed to parse Agent Fabric JSON header; ignoring'
    );
    return undefined;
  }
}

// The @agent-fabric/agents-ui-cloud widget synthesizes a client-side identity
