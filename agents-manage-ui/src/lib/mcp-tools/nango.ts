...
async function updateMCPGenericIntegration({
  ...
  const clientName = process.env.OAUTH_CLIENT_NAME || 'Agent Fabric';
  const clientUri = process.env.OAUTH_CLIENT_URI || 'http://localhost';
  const logoUri = process.env.OAUTH_CLIENT_LOGO_URI || '/logo.svg';
