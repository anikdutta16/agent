 ...
 * 2. Load and validate configuration from agent-fabric.config.ts
...
export async function initializeCommand(
  ...
  try {
    ...
    if (ciDetection.isCI) {
      ...
      if (logConfig && !quiet) {
        if (ciConfig) {
          ...
          console.log(chalk.gray(`  Using config file settings (no AGENT_FABRIC_API_KEY set)`));
    ...
    if (error.message.includes('Profile') && error.message.includes('not found')) {
      console.log(
        chalk.yellow('\nHint: Run "agent-fabric profile list" to see available profiles.')
      );
    } else if (error.message.includes('No configuration found')) {
      console.log(chalk.yellow('\nHint: Create a configuration file by running:'));
      console.log(chalk.gray('  agent-fabric init'));
      ...
      console.log(
        chalk.yellow('\nHint: Ensure your agent-fabric.config.ts has all required fields:')
      );
