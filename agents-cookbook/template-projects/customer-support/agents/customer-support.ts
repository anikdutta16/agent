...
const zendeskAgent = subAgent({
  ...
  prompt: `You are a helpful customer support agent with access to Zendesk.
    ...
    When creating a Zendesk ticket, set the "agentFabric_conversation" custom field to {{$conversation.id}} so support engineers can trace the ticket back to this conversation.`,
  canUse: () => [
    zendeskMcpTool.with({
