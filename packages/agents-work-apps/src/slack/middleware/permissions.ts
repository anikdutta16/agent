...
import { registerAuthzMeta } from '@agent-fabric/agents-core/middleware';
...
/**
 * Middleware that requires Agent Fabric org admin/owner role.
 * Use for workspace-level settings that only Agent Fabric organization admins can modify.
 */
...
>() => {
  ...
  registerAuthzMeta(mw, {
    ...
    permission: 'admin',
    description: 'Requires Agent Fabric org admin/owner role to modify workspace settings',
  });
  return mw;
};
...
>() => {
  const mw = createMiddleware<Env>(async (c: Context, next: Next) => {
    ...
    const userMappings = await findWorkAppSlackUserMappingByAgentFabricUserId(runDbClient)(userId);
    ...
    if (!userMapping) {
      throw createApiError({
        code: 'forbidden',
        message:
          'You must link your Slack account to modify channel settings. Use /agent-fabric link in Slack.',
        instance: c.req.path,
      });
    }
  ...
  registerAuthzMeta(mw, {
    description:
      'Requires Agent Fabric organization admin role, or Slack channel membership to modify channel settings',
  });
  return mw;
};
