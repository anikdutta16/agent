...
export async function profileListCommand(): Promise<void> {
  try {
    ...
    if (profiles.length === 0) {
      ...
      console.log(chalk.gray('Run "agent-fabric profile add <name>" to create one.'));
      return;
    }
...
export async function profileAddCommand(name?: string): Promise<void> {
  try {
    ...
    const remoteType = await p.select({
      message: 'Remote type:',
      options: [
        { value: 'cloud', label: 'Agent Fabric Cloud', hint: 'Default cloud deployment' },
    ...
    if (remoteType === 'local') {
      ...
      const credentialDefault = `agent-fabric-${profileName}`;
    ...
    if (credential !== 'none') {
      ...
      if (!credentialExists) {
        ...
        console.log(
          chalk.gray('  Run "agent-fabric login" to authenticate and store credentials.')
        );
      }
    }
...
export async function profileUseCommand(name: string): Promise<void> {
  try {
    if (!name) {
      ...
      console.log(chalk.gray('Usage: agent-fabric profile use <name>'));
      process.exit(1);
    }
...
export async function profileRemoveCommand(name: string): Promise<void> {
  try {
    if (!name) {
      console.error(chalk.red('Profile name is required.'));
      console.log(chalk.gray('Usage: agent-fabric profile remove <name>'));
      process.exit(1);
    }
  ...
  if (error instanceof ProfileError) {
    ...
    switch (error.code) {
      case 'PROFILE_NOT_FOUND':
        console.log(chalk.gray('\nRun "agent-fabric profile list" to see available profiles.'));
        break;
      case 'PROFILE_EXISTS':
        console.log(chalk.gray('\nUse a different name or remove the existing profile first.'));
        break;
      case 'ACTIVE_PROFILE_DELETE':
        console.log(chalk.gray('\nRun "agent-fabric profile use <other-profile>" first.'));
        break;
