import { ModelFactory, type ModelSettings } from '@agent-fabric/agents-core';
 ...
 * builder.
 */
export const AGENT_FABRIC_CACHE_BOUNDARY_PROP = '_agentFabricCacheBoundary';
 ...
 */
export const SYSTEM_CACHE_BOUNDARY_SENTINEL = '<<<AGENT_FABRIC_SYSTEM_CACHE_BOUNDARY>>>';
  ...
  const out = messages.map((msg) => {
    ...
    if (Array.isArray(msg.content)) {
      ...
      const content = (msg.content as unknown[]).map((part) => {
        if (part && typeof part === 'object' && AGENT_FABRIC_CACHE_BOUNDARY_PROP in part) {
          msgChanged = true;
          const { [AGENT_FABRIC_CACHE_BOUNDARY_PROP]: _drop, ...rest } = part as Record<
            string,
            unknown
          >;
