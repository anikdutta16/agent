...
function ConsentForm() {
  ...
  useEffect(() => {
    ...
    fetch(
      `${PUBLIC_AGENT_FABRIC_AGENTS_API_URL}/api/auth/oauth2/public-client?client_id=${encodeURIComponent(clientId)}`,
      { credentials: 'include' }
    )
  ...
  }, [isAuthenticated, clientId, PUBLIC_AGENT_FABRIC_AGENTS_API_URL]);
  ...
  const handleConsent = async (accept: boolean) => {
    ...
    try {
      const response = await fetch(
        `${PUBLIC_AGENT_FABRIC_AGENTS_API_URL}/api/auth/oauth2/consent`,
        {
          method: 'POST',
