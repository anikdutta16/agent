...
export const findWorkAppSlackUserMappingByAgentFabricUserId =
  (db: AgentsRunDatabaseClient) =>
  async (agentFabricUserId: string): Promise<WorkAppSlackUserMappingSelect[]> => {
    return db
      .select()
      .from(workAppSlackUserMappings)
      .where(eq(workAppSlackUserMappings.agentFabricUserId, agentFabricUserId));
  };
