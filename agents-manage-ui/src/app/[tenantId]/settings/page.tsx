...
import type { AllowedAuthMethod } from '@agent-fabric/agents-core/auth/auth-types';
import { parseAllowedAuthMethods } from '@agent-fabric/agents-core/auth/auth-types';
import { DEFAULT_MEMBERSHIP_LIMIT } from '@agent-fabric/agents-core/client-exports';
...
export default function SettingsPage({ params }: PageProps<'/[tenantId]/settings'>) {
  ...
  const { PUBLIC_IS_AGENT_FABRIC_CLOUD_DEPLOYMENT } = useRuntimeConfig();
  const isCloudDeployment = PUBLIC_IS_AGENT_FABRIC_CLOUD_DEPLOYMENT === 'true';
      ...
      <Card className="shadow-none">
        <CardHeader>
          ...
          <CardDescription>
            ...
            your Agent Fabric account, so actions are limited to your own permissions. No API key
            required.
          </CardDescription>
        </CardHeader>
        <CardContent>
