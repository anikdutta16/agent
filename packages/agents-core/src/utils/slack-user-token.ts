...
const ISSUER = 'agent-fabric-auth';
const AUDIENCE = 'agent-fabric-api';
const TOKEN_USE = 'slackUser';
const ACTOR_SUB = 'agent-fabric-work-app-slack';
const TOKEN_TTL = '1h';
...
export async function signSlackUserToken(params: SignSlackUserTokenParams): Promise<string> {
  try {
    const token = await signJwt({
      issuer: ISSUER,
      subject: params.agentFabricUserId,
      audience: AUDIENCE,
      expiresIn: TOKEN_TTL,
      claims: {
        tokenUse: TOKEN_USE,
    ...
    logger.debug(
      {
        agentFabricUserId: params.agentFabricUserId,
        tenantId: params.tenantId,
...
export async function verifySlackUserToken(token: string): Promise<VerifySlackUserTokenResult> {
  ...
  logger.debug(
    {
      agentFabricUserId: parseResult.data.sub,
