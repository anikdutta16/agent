...
export function RegisterSSOForm({
  ...
  const callbackUrl = buildCallbackUrl(PUBLIC_AGENT_FABRIC_AGENTS_API_URL, providerId);
...
export function EditSSOForm({
  ...
  const authClient = useAuthClient();
  const { PUBLIC_AGENT_FABRIC_AGENTS_API_URL } = useRuntimeConfig();
  const callbackUrl = buildCallbackUrl(PUBLIC_AGENT_FABRIC_AGENTS_API_URL, provider.providerId);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
