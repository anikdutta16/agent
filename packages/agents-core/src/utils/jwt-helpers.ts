...
export function getJwtSecret(): Uint8Array {
  const secret = env.AGENT_FABRIC_AGENTS_JWT_SIGNING_SECRET;

  if (!secret) {
    if (env.ENVIRONMENT === 'production') {
      throw new Error(
        'AGENT_FABRIC_AGENTS_JWT_SIGNING_SECRET environment variable is required in production'
      );
    }

    logger.warn(
      'AGENT_FABRIC_AGENTS_JWT_SIGNING_SECRET not set, using insecure default. DO NOT USE IN PRODUCTION!'
    );
