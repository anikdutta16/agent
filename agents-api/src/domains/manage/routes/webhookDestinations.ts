...
import { FileSecurityError } from '@agent-fabric/agents-core/external-fetch';
import { createProtectedRoute } from '@agent-fabric/agents-core/middleware';
import { getSlackClient, resolveWorkspaceToken } from '@agent-fabric/agents-work-apps/slack';
...
async function validateSlackChannelId(tenantId: string, slackChannelId: string): Promise<void> {
  ...
  try {
    ...
    if (!result.channel?.is_member) {
      throw createApiError({
        code: 'bad_request',
        message: `The Agent Fabric bot is not a member of this channel. Add the bot to the channel first.`,
      });
    }
  } catch (err) {
...
app.openapi(
  ...
  async (c) => {
    ...
    const slackCtx: SlackContext = {
      ...
      agentId: 'test-agent-id',
      agentName: 'Test Agent',
      manageUiBaseUrl: env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL || 'http://localhost:3000',
    };
    ...
    try {
      const response = await fetchWithSsrfProtection(dest.url, {
        ...
        headers: {
          ...
          'User-Agent': 'Agent Fabric-Webhooks/1.0',
        },
