...
cleanupStreamChunksHandler.openapi(
  createProtectedRoute({
    ...
    description:
      'Called by Vercel Cron to delete stream chunks older than 5 minutes. Auth via CRON_SECRET or AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET.',
    permission: noAuth(),
    security: [],
    responses: {
  ...
  async (c) => {
    ...
    const validSecret = env.CRON_SECRET ?? env.AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET;
