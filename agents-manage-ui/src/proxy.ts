import { isSessionCookie } from '@agent-fabric/agents-core/auth/cookie-names';
import { isDevelopment } from '@agent-fabric/agents-core/utils/env-detection';
...
export function buildCsp(config: ReturnType<typeof getRuntimeConfig>, sentryDsn?: string): string {
  ...
  const recaptchaScriptOrigin = config.PUBLIC_AGENT_FABRIC_RECAPTCHA_SITE_KEY
    ? 'https://www.google.com/recaptcha/'
    : null;
  const recaptchaStaticOrigin = config.PUBLIC_AGENT_FABRIC_RECAPTCHA_SITE_KEY
    ? 'https://www.gstatic.com/recaptcha/'
    : null;

  const connectSrcDomains = [
    "'self'",
    config.PUBLIC_AGENT_FABRIC_AGENTS_API_URL,
    posthogHost,
...
async function tryDevAutoLogin(): Promise<string | null> {
  const apiUrl = runtimeConfig.PUBLIC_AGENT_FABRIC_AGENTS_API_URL;
  const bypassSecret = process.env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET;

  if (!bypassSecret) {
    console.warn(
      '[proxy] AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET is not set — dev auto-login will not work. ' +
