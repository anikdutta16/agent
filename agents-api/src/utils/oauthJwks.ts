import { getInProcessFetch } from '@agent-fabric/agents-core';
...
export function getOAuthJwks(): ReturnType<typeof createRemoteJWKSet> {
  if (!cachedJwks) {
    const jwksUrl = new URL(
      '/api/auth/jwks',
      env.AGENT_FABRIC_AGENTS_API_URL || 'http://localhost:3002'
    );
...
export function getOAuthIssuer(): string {
  return `${env.AGENT_FABRIC_AGENTS_API_URL || 'http://localhost:3002'}/api/auth`;
}
 ...
 * independent sources (`AGENT_FABRIC_AGENTS_API_URL` here vs `config.baseURL` there). A
...
export function getAcceptedAudiences(): string[] {
  const base = (env.AGENT_FABRIC_AGENTS_API_URL || 'http://localhost:3002').replace(/\/+$/, '');
