...
/**
 * Slack user mappings - maps Slack users to Agent Fabric users.
...
export const workAppSlackUserMappings = pgTable(
  'work_app_slack_user_mappings',
  {
    ...
    slackTeamId: varchar('slack_team_id', { length: 256 }).notNull(),
    slackEnterpriseId: varchar('slack_enterprise_id', { length: 256 }),
    agentFabricUserId: text('agentFabric_user_id')
      .notNull()
  ...
  (table) => [
    ...
    index('work_app_slack_user_mappings_user_idx').on(table.agentFabricUserId),
