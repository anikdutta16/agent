import { deriveKidFromPublicKey, getAppById, updateApp } from '@agent-fabric/agents-core';
...
function derivePlaygroundDomains(): string[] {
  const manageUiUrl = env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL;
  ...
  if (manageUiUrl) {
    try {
      ...
      logger.warn(
        { manageUiUrl },
        'Invalid AGENT_FABRIC_AGENTS_MANAGE_UI_URL, falling back to environment defaults'
      );
    }
  }
  ...
  if (env.ENVIRONMENT === 'production') {
    logger.error(
      {},
      'AGENT_FABRIC_AGENTS_MANAGE_UI_URL not set in production — cannot derive playground allowed domains'
    );
    return [];
  }
...
export async function ensurePlaygroundAppConfig(): Promise<void> {
  const appId = env.AGENT_FABRIC_PLAYGROUND_APP_ID || 'app_playground';
  ...
  const publicKeyB64 = env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY;
  if (publicKeyB64) {
