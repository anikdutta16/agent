...
export async function detectCIEnvironment(): Promise<CIDetectionResult> {
  ...
  if (process.env.AGENT_FABRIC_CI === 'true') {
    return { isCI: true, reason: 'AGENT_FABRIC_CI=true' };
  }
  ...
  const profilesPath = join(homedir(), '.agent-fabric', 'profiles.yaml');
  ...
  // If keychain unavailable and AGENT_FABRIC_API_KEY is set, treat as CI
  if (!keychainAvailable && process.env.AGENT_FABRIC_API_KEY) {
...
export function loadCIEnvironmentConfig(): CIEnvironmentConfig | null {
  const apiKey = process.env.AGENT_FABRIC_API_KEY;
  const agentsApiUrl = process.env.AGENT_FABRIC_AGENTS_API_URL;
  const environment = process.env.AGENT_FABRIC_ENVIRONMENT || 'production';
  const tenantId = process.env.AGENT_FABRIC_TENANT_ID;
  ...
  return {
    isCI: true,
    apiKey,
    agentsApiUrl: agentsApiUrl || 'http://localhost:3002',
    environment,
    tenantId,
  };
}
...
export function logCIConfig(config: CIEnvironmentConfig, reason: string): void {
  ...
  console.log(chalk.gray(`  Auth: API key (AGENT_FABRIC_API_KEY)`));
  if (config.tenantId) {
...
export function validateCIConfig(config: CIEnvironmentConfig): {
  ...
  if (!config.apiKey) {
    errors.push('AGENT_FABRIC_API_KEY environment variable is required in CI mode');
  }
