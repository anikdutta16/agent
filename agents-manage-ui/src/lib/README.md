...
This library provides server actions and types for interacting with the AgentFull API from the agent-fabric-chat backend. It imports the original schemas from the agent-fabric-chat package to avoid duplication and ensure consistency.
...
```bash
AGENT_FABRIC_AGENTS_API_URL="http://localhost:3002"
```
