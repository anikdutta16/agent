...
import { signSlackUserToken } from '@agent-fabric/agents-core';
import { env } from '../../../env';
...
export async function handleAppMention(params: {
  ...
  return tracer.startActiveSpan(SLACK_SPAN_NAMES.APP_MENTION, async (span) => {
    ...
    const manageUiUrl = env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL || 'http://localhost:3000';
    ...
    if (!tenantId) {
      ...
      await slackClient.chat
        .postEphemeral({
          channel,
          user: slackUserId,
          text: 'This workspace is not properly configured. Please reinstall the Slack app from the Agent Fabric dashboard.',
        })
        .catch((e) =>
    ...
    try {
      ...
      if (!existingLink) {
        ...
        await slackClient.chat.postEphemeral({
          ...
          user: slackUserId,
          thread_ts: isInThread ? threadTs : undefined,
          text: "To get started, let's connect your Agent Fabric account with Slack.",
      ...
      if (isInThread && !hasQuery) {
        ...
        if (isBotThread) {
          ...
          await slackClient.chat.postEphemeral({
            ...
            text:
              ...
              `Or use \`@Agent Fabric <prompt>\` to start a new prompt.`,
          });
          span.end();
          return;
        }
        ...
        if (!contextMessages) {
          ...
          await slackClient.chat.postEphemeral({
            ...
            user: slackUserId,
            thread_ts: threadTs,
            text: `Unable to retrieve thread context. Try using \`@Agent Fabric <your question>\` instead.`,
          });
          span.end();
          return;
        }
        ...
        const slackUserToken = await signSlackUserToken({
          agentFabricUserId: existingLink.agentFabricUserId,
          tenantId,
          slackTeamId: teamId,
          slackUserId,
