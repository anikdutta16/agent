...
Projects are the top-level organizational unit in your Agent Fabric workspace. Each project contains its own agents, sub agents, tools, data components, and environment configurations.
...
├── src/  
│   ├── agent-fabric.config.ts         # Workspace configuration file
│   ├── projects/
│   │   ├── my-agent-project/
│   │   │   ├── index.ts
│   │   │   ├── agents/
...
The `src/agent-fabric.config.ts` file defines workspace configuration for all projects. See [Workspace Configuration](/typescript-sdk/workspace-configuration) for details.
...
- **MCP Service** (Port 3006): Custom MCP servers
  - Custom MCP servers for external tools and APIs. See [our guide](https://localhost/tutorials/mcp-servers/custom-mcp-servers#getting-started) for how to add MCP servers to your project.
...
See [Agents & Sub Agents](https://localhost/typescript-sdk/agent-settings) for more information about creating and configuring agents and sub agents.
...
See [MCP Tools](https://localhost/typescript-sdk/tools/mcp-tools) and [Function Tools](https://localhost/typescript-sdk/tools/function-tools) for more information about creating and configuring MCP tools and function tools.
...
import { dataComponent } from '@agent-fabric/agents-sdk';
import { z } from 'zod';
...
See [Data Components](https://localhost/typescript-sdk/structured-outputs/data-components) for more information about creating and configuring data components.
...
```typescript
import { externalAgent } from '@agent-fabric/agents-sdk';
...
See [External Agents](https://localhost/typescript-sdk/external-agents) for more information about creating and configuring external agents.
...
import { CredentialStoreType } from '@agent-fabric/agents-core';
...
Use the `agent-fabric add` command to add template projects from the [Agent Fabric Cookbook](https://github.com/agent-fabric/agents/tree/main/agents-cookbook/template-projects) to your workspace.

### Prerequisite

Ensure you have the [Agent Fabric CLI](/typescript-sdk/cli-reference) installed.
...
To see all available project templates, run `agent-fabric add` without any arguments:
...
```bash
agent-fabric add --project docs-assistant
```
...
After making changes to your project code, use `agent-fabric push` to sync your local TypeScript project to the Visual Builder. This allows you to continue development using the Visual Builder's drag-and-drop interface.
...
```bash
agent-fabric push --json
```

## Pulling changes from the Visual Builder

When you make changes in the Visual Builder (such as updating prompts or modifying agent configurations), use `agent-fabric pull` to sync those changes back to your local TypeScript project.

### Prerequisite

The `agent-fabric pull` command in-part leverages AI to sync your TypeScript files to the state of your Visual Builder, so **at least one** of the below environment variables to be defined:
...
See [CLI Reference](/typescript-sdk/cli-reference#agent-fabric-pull) for more information about the `agent-fabric pull` command.

## How to add a new MCP server

Use the `agent-fabric add` command to add custom MCP server templates from the [Agent Fabric Cookbook](https://github.com/agent-fabric/agents/tree/main/agents-cookbook/template-mcps) to your workspace. These templates provide starter code for common MCP server integrations that you can customize and deploy.
...
To see all available MCP server templates, run `agent-fabric add` without any arguments:
...
To build and run your own images you can follow the [Build a Custom Docker Image](https://localhost/deployment/docker-build) docs.
...
- [Deploy to Vercel](https://localhost/deployment/vercel)
- [Docker (Local Dev)](https://localhost/deployment/docker-local)
- [AWS EC2](https://localhost/deployment/aws-ec2)
- [Azure VM](https://localhost/deployment/azure-vm)
- [GCP Compute Engine](https://localhost/deployment/gcp-compute-engine)
- [Hetzner](https://localhost/deployment/hetzner)
