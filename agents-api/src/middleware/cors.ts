...
/**
 * Extract the base domain from a hostname (e.g., 'app.preview.localhost' -> 'preview.localhost')
 */
 ...
 * e.g., 'api.localhost' -> 'localhost', 'app.localhost' -> 'localhost'
 */
...
export function isOriginAllowed(origin: string | undefined): origin is string {
  ...
  try {
    ...
    const apiUrl = new URL(env.AGENT_FABRIC_AGENTS_API_URL || 'http://localhost:3002');
    const uiUrl = env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL
      ? new URL(env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL)
      : null;
    ...
    // the API and UI don't share a 3-part parent (e.g., api.localhost + app.localhost).
    if (uiUrl) {
