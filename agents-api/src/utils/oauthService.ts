...
import type { McpTokenExchangeResult } from '@agent-fabric/agents-core';
import { exchangeMcpAuthorizationCode, initiateMcpOAuthFlow } from '@agent-fabric/agents-core';
import { env } from '../env';
...
class OAuthService {
  ...
  constructor(config: OAuthClientConfig = {}) {
    this.defaultConfig = {
      ...
      clientName: config.clientName || process.env.OAUTH_CLIENT_NAME || 'Agent Fabric',
      clientUri: config.clientUri || process.env.OAUTH_CLIENT_URI || 'http://localhost',
      logoUri: config.logoUri || process.env.OAUTH_CLIENT_LOGO_URI || '/logo.svg',
      redirectBaseUrl: config.redirectBaseUrl || env.AGENT_FABRIC_AGENTS_API_URL,
    };
  }
