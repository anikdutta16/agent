import { getInProcessFetch, getWaitUntil, registerAppFetch } from '@agent-fabric/agents-core';
import { githubRoutes } from '@agent-fabric/agents-work-apps/github';
import {
  ...
  if (auth) {
    ...
    if (env.ENVIRONMENT === 'development') {
      app.post('/api/auth/dev-session', async (c) => {
        const bypassSecret = env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET;
        const authHeader = c.req.header('Authorization');
        if (!bypassSecret || authHeader !== `Bearer ${bypassSecret}`) {
          return c.json({ error: 'Unauthorized' }, 401);
        }

        const email = env.AGENT_FABRIC_AGENTS_MANAGE_UI_USERNAME;
