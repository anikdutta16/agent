...
export async function loginCommand(options: LoginOptions = {}): Promise<void> {
  ...
  try {
    ...
    profileName = 'default';
    credentialKey = 'agent-fabric-cloud';
  ...
  if (!available) {
    ...
    console.log(
      chalk.gray('  Set AGENT_FABRIC_API_KEY environment variable instead of using login.')
    );
    console.log(chalk.gray('  See: https://localhost/cli/cicd'));
    process.exit(1);
  }
  ...
  try {
    ...
    const deviceCodeResponse = await fetch(`${manageApiUrl}/api/auth/device/code`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'User-Agent': USER_AGENT },
      body: JSON.stringify({ client_id: 'agent-fabric-cli' }),
    });
