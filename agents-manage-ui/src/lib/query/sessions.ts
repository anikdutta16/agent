...
// agents-cli sends `agent-fabric-cli/<version> node/<v> <platform>/<arch>` so its sessions
...
const AGENT_FABRIC_CLI_UA = /^agent-fabric-cli\/([\d.]+)/i;

export function parseDeviceDescriptor(userAgent: string | null | undefined): string {
  if (!userAgent) return 'Unknown device';
  const cliMatch = userAgent.match(AGENT_FABRIC_CLI_UA);
  if (cliMatch) return `Agent Fabric CLI ${cliMatch[1]}`;
  try {
