...
function resolveWebRuntime(isRoot = false) {
  try {
    ...
    const pkg = require.resolve('@agent-fabric/agents-manage-ui/package.json');
    ...
    throw new Error(
      `Could not find @agent-fabric/agents-manage-ui package. Please install it first:\n\n` +
        `  npm install @agent-fabric/agents-manage-ui\n` +
        `  # or\n` +
        `  pnpm add @agent-fabric/agents-manage-ui\n\n` +
...
async function buildNextApp({ outputDir }: { outputDir: string }) {
  ...
  try {
    ...
    if (!existsSync(standalonePath)) {
      ...
      console.error(chalk.gray('  cd node_modules/@agent-fabric/agents-manage-ui'));
...
async function exportNextApp({ outputDir }: { outputDir: string }) {
  ...
  try {
    ...
    if (!existsSync(root)) {
      ...
      console.error(chalk.red('The @agent-fabric/agents-manage-ui package was not found.'));
      console.error(chalk.yellow('Please install it first:'));
      console.error(chalk.gray('  npm install @agent-fabric/agents-manage-ui'));
      console.error(chalk.gray('  # or'));
      console.error(chalk.gray('  pnpm add @agent-fabric/agents-manage-ui'));
      process.exit(1);
    }
