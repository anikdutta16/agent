import { KeyChainStore } from '@agent-fabric/agents-core/credential-stores';

const CLI_SERVICE_PREFIX = 'agent-fabric-cli';
...
function getKeychainUnavailableMessage(reason?: string): string {
  ...
  message +=
    'The Agent Fabric CLI requires access to the system keychain to store your login credentials securely.\n\n';
  ...
  switch (platform) {
    ...
    case 'linux':
      ...
      message +=
        '  3. For headless servers, consider using an API key instead of `agent-fabric login`\n';
      break;
    default:
      message +=
