...
const REQUIRED_ENV_VARS = [
  ...
  'LANGFUSE_SECRET_KEY',
  'LANGFUSE_BASE_URL',
  'AGENT_FABRIC_AGENTS_RUN_API_KEY',
  'AGENT_FABRIC_AGENTS_RUN_API_URL',
] as const;
...
function parseAndValidateConfig(): RunConfig {
  const config: Partial<RunConfig> = {
    tenantId: process.env.AGENT_FABRIC_TENANT_ID,
    projectId: process.env.AGENT_FABRIC_PROJECT_ID,
    agentId: process.env.AGENT_FABRIC_AGENT_ID,
    runName: process.env.AGENT_FABRIC_RUN_NAME,
    baseUrl: process.env.AGENT_FABRIC_AGENTS_RUN_API_URL,
    apiKey: process.env.AGENT_FABRIC_AGENTS_RUN_API_KEY,
    datasetName: 'agent-fabric-weather-example-dataset',
  };
...
async function runDatasetEvaluation(config: RunConfig): Promise<void> {
  ...
  const authKey = apiKey || process.env.AGENT_FABRIC_AGENTS_RUN_API_KEY;
  if (!authKey) throw new Error('API key is required. Set AGENT_FABRIC_AGENTS_RUN_API_KEY');
  if (!baseUrl) throw new Error('Base URL is required. Set AGENT_FABRIC_AGENTS_RUN_API_URL');
  ...
  try {
    ...
    if (result.traceId) {
      ...
      await item.link(traceRef, context.runLabel, {
        description: 'Dataset run via Agent Fabric',
    ...
    return tracer.startActiveSpan('chat-api-call', async (span) => {
      try {
        ...
        const headers: Record<string, string> = {
          ...
          'x-agent-fabric-tenant-id': this.ctx.tenantId,
          'x-agent-fabric-project-id': this.ctx.projectId,
          'x-agent-fabric-agent-id': this.ctx.agentId,
        };
