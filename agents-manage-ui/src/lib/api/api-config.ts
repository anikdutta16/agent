...
import { isAuthCookie } from '@agent-fabric/agents-core/auth/cookie-names';
import { DEFAULT_AGENT_FABRIC_AGENTS_API_URL } from '../runtime-config/defaults';
import { ApiError } from '../types/errors';

// Lazy initialization with runtime warnings
let AGENT_FABRIC_AGENTS_API_URL: string | null = null;
let hasWarnedAgentsApi = false;

export function getAgentsApiUrl(): string {
  if (AGENT_FABRIC_AGENTS_API_URL === null) {
    AGENT_FABRIC_AGENTS_API_URL =
      process.env.AGENT_FABRIC_AGENTS_API_URL || DEFAULT_AGENT_FABRIC_AGENTS_API_URL;

    if (!process.env.AGENT_FABRIC_AGENTS_API_URL && !hasWarnedAgentsApi) {
      console.warn(
        `AGENT_FABRIC_AGENTS_API_URL is not set, falling back to: ${DEFAULT_AGENT_FABRIC_AGENTS_API_URL}`
      );
      hasWarnedAgentsApi = true;
    }
  }
  return AGENT_FABRIC_AGENTS_API_URL;
}
...
async function makeApiRequestInternal<T>(
  ...
  const hasBypassSecret = Boolean(process.env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET);
  const authMode = hasBypassSecret ? 'bypass-secret' : cookieHeader ? 'cookie' : 'none';

  if (authMode === 'none') {
    console.warn(
      `[api-config] No auth credentials for ${endpoint} - ` +
        'set AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET or ensure session cookies are forwarded'
    );
  }
  ...
  const defaultHeaders: HeadersInit = {
    ...
    ...(hasBypassSecret && {
      Authorization: `Bearer ${process.env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET}`,
    }),
  };
