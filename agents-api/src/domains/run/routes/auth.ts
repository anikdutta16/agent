...
import { createProtectedRoute, noAuth } from '@agent-fabric/agents-core/middleware';
...
export function getAnonJwtSecret(): Uint8Array {
  const secret = env.AGENT_FABRIC_ANON_JWT_SECRET;

  if (!secret) {
    if (env.ENVIRONMENT !== 'development' && env.ENVIRONMENT !== 'test') {
      throw new Error('AGENT_FABRIC_ANON_JWT_SECRET environment variable is required');
    }
    logger.warn(
      {},
      'Using random ephemeral secret for anonymous JWTs — set AGENT_FABRIC_ANON_JWT_SECRET'
    );
...
async function proxyChallengeGet(
  ...
  const apiKeyId = env.AGENT_FABRIC_SENTINEL_API_KEY_ID;
  const baseUrl = env.AGENT_FABRIC_SENTINEL_BASE_URL;
  ...
  if (endpointClass) {
    headers['x-agent-fabric-endpoint-class'] = endpointClass;
  }
...
// Legacy compatibility: embedded widgets pinned to @agent-fabric/agents-ui versions that predate the
...
app.openapi(
  ...
  async (c) => {
    const apiKeyId = env.AGENT_FABRIC_SENTINEL_V1_API_KEY_ID;
    const baseUrl = env.AGENT_FABRIC_SENTINEL_BASE_URL;
    if (!isSentinelEnabled(apiKeyId) || !baseUrl) {
...
app.openapi(
  ...
  async (c) => {
    ...
    const sentinelKeyId = env.AGENT_FABRIC_SENTINEL_API_KEY_ID;
    const sentinelKeySecret = env.AGENT_FABRIC_SENTINEL_API_KEY_SECRET;
    const sentinelBaseUrl = env.AGENT_FABRIC_SENTINEL_BASE_URL;
    const v1KeySecret = env.AGENT_FABRIC_SENTINEL_V1_API_KEY_SECRET;
    ...
    const sentinelV1Enabled =
      isSentinelEnabled(env.AGENT_FABRIC_SENTINEL_V1_API_KEY_ID) && !!v1KeySecret;
    ...
    // AGENT_FABRIC_SENTINEL_V1_* env vars. Once v1 is disabled, v1-shaped headers return 400.
    if (sentinelV2Enabled && sentinelV1Enabled && !dualSentinelWarned) {
      dualSentinelWarned = true;
      logger.warn(
        {},
        'Sentinel v1 and v2 are both enabled. Clients choose the verification path by envelope shape; v1 has no replay protection or HIS scoring. Disable AGENT_FABRIC_SENTINEL_V1_* once legacy widget traffic drops to ~0.'
      );
    }

    if (sentinelV2Enabled || sentinelV1Enabled) {
      const challengeHeader = c.req.header('x-agent-fabric-challenge-solution');
      if (!challengeHeader) {
        throw createApiError({
          code: 'bad_request',
    ...
    const lifetimeSeconds = env.AGENT_FABRIC_ANON_SESSION_LIFETIME_SECONDS;
