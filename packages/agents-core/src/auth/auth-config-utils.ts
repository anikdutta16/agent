...
export async function getTrustedOrigins(
  ...
  const staticOrigins = [
    'http://localhost:3000',
    'http://localhost:3002',
    env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL,
 ...
 *
 * When the API and UI share a common 3-part parent (e.g., api.pilot.localhost
 * and pilot.localhost both share .pilot.localhost), the function auto-computes
 * the shared parent. When domains don't share a 3-part parent (e.g.,
 * api.localhost and app.localhost), set AUTH_COOKIE_DOMAIN explicitly.
 *
 * Examples (auto-computed from baseURL):
 * - https://api.pilot.localhost -> .pilot.localhost
 * - https://pilot.localhost -> .pilot.localhost
 * - http://localhost:3002 -> undefined (no domain for localhost)
 *
 * With AUTH_COOKIE_DOMAIN=.localhost:
 * - Any *.localhost URL -> .localhost
 */
