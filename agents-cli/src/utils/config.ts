 ...
 */
function normalizeConfig(config: any): AgentFabricConfig {
...
export function getConfigFileNames(tag?: string): string[] {
  if (tag) {
    // Tag-based config files: <tag>.__agent-fabric.config.ts__
    return [`${tag}.__agent-fabric.config.ts__`, `${tag}.__agent-fabric.config.js__`];
  }
  // Default config file names
  return [
    'agent-fabric.config.ts',
    'agent-fabric.config.js',
    '.agentFabricrc.ts',
    '.agentFabricrc.js',
  ];
}
...
export async function loadConfigFromFile(
  configPath?: string,
  tag?: string
): Promise<AgentFabricConfig | null> {
...
/**
 * Main config loader - single source of truth for loading agent-fabric.config.ts
 ...
 * 2. Config file (agent-fabric.config.ts)
 * 3. Default values
 *
 * @param configPath - Optional explicit path to config file
 * @param tag - Optional tag for environment-specific config (e.g., 'prod', 'staging')
 ...
 */
export async function loadConfig(configPath?: string, tag?: string): Promise<AgentFabricConfig> {
  ...
  const config: AgentFabricConfig = {
  ...
  if (fileConfig) {
    ...
    Object.keys(fileConfig).forEach((key) => {
      const value = fileConfig[key as keyof AgentFabricConfig];
      if (value !== undefined) {
...
/**
 * Validates configuration loaded from agent-fabric.config.ts file
 * This is the ONLY way to configure the CLI - no CLI flags for URLs/keys
 *
 * Configuration priority:
 * 1. Config file (agent-fabric.config.ts or --config path/to/config.ts)
 * 2. CLI credentials (from `agent-fabric login`) - for API key and tenant ID fallback
...
export async function validateConfiguration(
  ...
  // Login stores under 'agent-fabric-cloud'; default loadCredentials() uses 'auth-credentials'
  if (!config.agentsApiKey && !cliCredentials) {
    const cloudCreds = await loadCredentials('agent-fabric-cloud');
    if (cloudCreds?.accessToken) {
      ...
      logger.info({}, 'Using CLI login credentials (agent-fabric-cloud)');
    }
  }
    ...
    if (actualConfigFile) {
      throw new Error(
        `Tenant ID is missing from configuration file: ${actualConfigFile}\n` +
          ...
          'or run "agent-fabric login" to authenticate with Agent Fabric Cloud.'
      );
    }
    throw new Error(
      'No configuration found. Please:\n' +
        '  1. Run "agent-fabric login" to authenticate with Agent Fabric Cloud\n' +
        '  2. Or create "agent-fabric.config.ts" by running "agent-fabric init"\n' +
