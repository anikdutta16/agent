...
function generateDevManifest(manifest: SlackManifest, devId: string): Record<string, unknown> {
  ...
  dev.display_information = {
    ...dev.display_information,
    name: `Agent Fabric Dev ${devId}`,
  };

  if (dev.features?.bot_user) {
    dev.features.bot_user = {
      ...dev.features.bot_user,
      display_name: `agent-fabric-${devId}`,
    };
  }
...
async function main(): Promise<void> {
  ...
  console.log(`  ${fmt.label('App name:')}  ${fmt.value(`Agent Fabric Dev ${devId}`)}`);
  console.log(`  ${fmt.label('Bot name:')}  ${fmt.value(`@agent-fabric-${devId}`)}\n`);
  ...
  console.log('');
  console.log(`  ${fmt.label('App:')}        ${fmt.value(`Agent Fabric Dev ${devId}`)}`);
  console.log(`  ${fmt.label('Bot:')}        ${fmt.value(`@agent-fabric-${devId}`)}`);
