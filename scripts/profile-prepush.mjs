...
function analyzeSummary(summaryPath) {
  ...
  for (const task of sortedTasks.slice(0, 10)) {
    ...
    const taskName = `${task.package.replace('@agent-fabric/', '')}#${task.task}`;
  ...
  for (const task of buildTasks.slice(0, 5)) {
    ...
    console.log(
      `    • ${task.package.replace('@agent-fabric/', '')}: ${formatDuration(task.duration)} (${pct}% of total time)`
    );
  }
  ...
  const docsBuild = tasks.find(
    (t) => t.package === '@agent-fabric/agents-docs' && t.task === 'build'
  );
  const uiBuild = tasks.find(
    (t) => t.package === '@agent-fabric/agents-manage-ui' && t.task === 'build'
  );
  ...
  if (docsBuild && docsBuild.duration > 30) {
    ...
    console.log(
      `     ${colors.dim}How: Add --filter='!@agent-fabric/agents-docs' to check:husky${colors.reset}\n`
    );
  }
  ...
  if (uiBuild && uiBuild.duration > 60) {
    ...
    console.log(
      `     ${colors.dim}How: Add --filter='!@agent-fabric/agents-manage-ui' to check:husky${colors.reset}\n`
    );
  }
  ...
  if (docsBuild && docsBuild.duration > 30) excludePackages.push('@agent-fabric/agents-docs');
