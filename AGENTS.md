...
# Run visual tests
pnpm --filter @agent-fabric/agents-manage-ui test:visual

# Update baselines after intentional UI changes
pnpm --filter @agent-fabric/agents-manage-ui test:visual:update
...
This is the **Agent Fabric** - a multi-agent AI system with A2A (Agent-to-Agent) communication capabilities. The system provides OpenAI Chat Completions compatible API while supporting sophisticated agent orchestration.
...
#### Shared Test Mocks (`@agent-fabric/agents-core/test-utils`)

Use the shared mock factories instead of defining inline mocks. This prevents drift when the real API changes.

```typescript
import { createMockLoggerModule } from '@agent-fabric/agents-core/test-utils';
...
vi.mock('../../logger', async () => {
  const { createMockLoggerModule } = await import('@agent-fabric/agents-core/test-utils');
...
AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL=postgresql://appuser:password@localhost:5432/agent_fabric
AGENT_FABRIC_AGENTS_RUN_DATABASE_URL=postgresql://appuser:password@localhost:5433/agent_fabric
PORT=3002
...
- The `env` command outputs `export` statements that override database URLs (`AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL`, `AGENT_FABRIC_AGENTS_RUN_DATABASE_URL`, `SPICEDB_ENDPOINT`) and app ports (`AGENTS_API_PORT`, `MANAGE_UI_PORT`, `AGENT_FABRIC_AGENTS_API_URL`)
...
Any code in `agents-api` or `agents-work-apps` that makes **internal A2A calls or self-referencing API calls** (i.e. calling another route on the same service) **MUST** use `getInProcessFetch()` from `@agent-fabric/agents-core` instead of the global `fetch`.
...
All API routes in `agents-api` **must** use `createProtectedRoute()` from `@agent-fabric/agents-core/middleware` instead of the plain `createRoute()` from `@hono/zod-openapi`. This is enforced by Biome lint rules and ensures every route has explicit authorization metadata (`x-authz`) in the OpenAPI spec.

```typescript
import { createProtectedRoute, noAuth, inheritedAuth } from '@agent-fabric/agents-core/middleware';
...
Replace `service` with the current service name (e.g., `agent-fabric-api` in prod, `agent-fabric-api-test` in tests). If using SigNoz/OTLP, point to that host/port instead of localhost.
...
curl "http://localhost:16686/api/operations?service=agent-fabric-api"

# Search traces for recent activity (last hour)
curl "http://localhost:16686/api/traces?service=agent-fabric-api&limit=20&lookback=1h"

# Search traces by operation name
curl "http://localhost:16686/api/traces?service=agent-fabric-api&operation=agent.generate&limit=10"

# Search traces by tags (useful for finding specific agent/conversation)
curl "http://localhost:16686/api/traces?service=agent-fabric-api&tags=%7B%22agent.id%22:%22qa-agent%22%7D&limit=10"

# Search traces by tags for conversation ID
curl "http://localhost:16686/api/traces?service=agent-fabric-api&tags=%7B%22conversation.id%22:%22conv-123%22%7D"

# Get specific trace by ID
curl "http://localhost:16686/api/traces/{trace-id}"

# Search for traces with errors
curl "http://localhost:16686/api/traces?service=agent-fabric-api&tags=%7B%22error%22:%22true%22%7D&limit=10"

# Search for tool call traces
curl "http://localhost:16686/api/traces?service=agent-fabric-api&operation=tool.call&limit=10"

# Search traces within time range (Unix timestamps)
curl "http://localhost:16686/api/traces?service=agent-fabric-api&start=1640995200000000&end=1641081600000000"
```

### Common Debugging Workflows

**Debugging Agent Transfers:**
1. View traces: `curl "http://localhost:16686/api/traces?service=agent-fabric-api&tags=%7B%22conversation.id%22:%22conv-123%22%7D"`

**Debugging Tool Calls:**
1. Find tool call traces: `curl "http://localhost:16686/api/traces?service=agent-fabric-api&operation=tool.call&limit=10"`

**Debugging Task Delegation:**
1. Trace execution flow: `curl "http://localhost:16686/api/traces?service=agent-fabric-api&tags=%7B%22task.id%22:%22task-id%22%7D"`

**Debugging Performance Issues:**
1. Find slow operations: `curl "http://localhost:16686/api/traces?service=agent-fabric-api&minDuration=5s"`
2. View error traces: `curl "http://localhost:16686/api/traces?service=agent-fabric-api&tags=%7B%22error%22:%22true%22%7D"`
...
- Build `@agent-fabric/agents-core` before `pnpm setup-dev` (setup imports its `dist/setup`). Then `pnpm dev` serves API `:3002` (`/health` 204) and Manage UI `:3000` (local-dev auto-login `admin@example.com`).
- `.env` needs `ANTHROPIC_API_KEY`; a placeholder unblocks DB/API/UI + project CRUD, real agent chat needs a valid key. `setup-dev`'s sample-project push may fail (`agent-fabric` bin not resolvable) but is non-fatal.
