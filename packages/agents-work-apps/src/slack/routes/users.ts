...
import {
  ...
  findWorkAppSlackUserMappingByAgentFabricUserId,
  flushTraces,
  getWaitUntil,
  isUniqueConstraintError,
  verifySlackLinkToken,
} from '@agent-fabric/agents-core';
import { createProtectedRoute, inheritedWorkAppsAuth } from '@agent-fabric/agents-core/middleware';
...
app.openapi(
  createProtectedRoute({
    ...
    description: 'Check if a Slack user is linked to an Agent Fabric account',
    ...
    try {
      ...
      if (!verifyResult.valid || !verifyResult.payload) {
        ...
        const errorMessage = isExpired
          ? 'Token expired. Please run /agent-fabric link in Slack to get a new one.'
          : verifyResult.error ||
            'Invalid or expired link token. Please run /agent-fabric link in Slack to get a new one.';
      ...
      const agentFabricUserId = sessionUserId;
      ...
      if (existingLink && existingLink.agentFabricUserId === agentFabricUserId) {
        logger.info(
          { slackUserId, tenantId, agentFabricUserId: body.userId },
      ...
      if (existingLink) {
        logger.info(
          {
            slackUserId,
            existingUserId: existingLink.agentFabricUserId,
            newUserId: agentFabricUserId,
            tenantId,
          },
      ...
      const slackUserMapping = await createWorkAppSlackUserMapping(runDbClient)({
        ...
        agentFabricUserId: agentFabricUserId,
      });
      ...
      logger.info(
        {
          ...
          slackTeamId: teamId,
          tenantId,
          agentFabricUserId: body.userId,
          linkId: slackUserMapping.id,
        },
        'Successfully linked Slack user to Agent Fabric account via JWT token'
      );
    ...
    request: {
      body: {
        content: {
          'application/json': {
            schema: z.object({
              userId: z.string().describe('Agent Fabric user ID'),
...
app.openapi(
  createProtectedRoute({
    ...
    description: 'Unlink a Slack user from their Agent Fabric account.',
    ...
    request: {
      body: {
        content: {
          'application/json': {
            schema: z.object({
              userId: z.string().optional().describe('Agent Fabric user ID'),
  ...
  async (c) => {
    ...
    try {
      ...
      if (slackUserId && slackTeamId) {
        ...
        if (!isAuthorizedForUser(c, mapping.agentFabricUserId)) {
          return c.json({ error: 'Can only disconnect your own account' }, 403);
        }
      ...
      if (userId) {
        const userMappings =
          await findWorkAppSlackUserMappingByAgentFabricUserId(runDbClient)(userId);
...
app.openapi(
  createProtectedRoute({
    ...
    description: 'Check if an Agent Fabric user has a linked Slack account.',
  ...
  async (c) => {
    ...
    try {
      const userMappings =
        await findWorkAppSlackUserMappingByAgentFabricUserId(runDbClient)(appUserId);
      ...
      const connection = {
        ...
        appUserId: mostRecent.agentFabricUserId,
