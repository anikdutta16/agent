...
export function getOAuthLoginUrl({
  PUBLIC_AGENT_FABRIC_AGENTS_API_URL,
  tenantId,
  projectId,
  id,
}: {
  PUBLIC_AGENT_FABRIC_AGENTS_API_URL: string;
  tenantId: string;
  projectId: string;
  id: string;
}): string {
  ...
  const url = `${PUBLIC_AGENT_FABRIC_AGENTS_API_URL}/manage/tenants/${encodeURIComponent(tenantId)}/projects/${encodeURIComponent(projectId)}/tools/${encodeURIComponent(id)}/oauth/login`;

  // Verify the final URL starts with our expected base
  if (!url.startsWith(PUBLIC_AGENT_FABRIC_AGENTS_API_URL)) {
