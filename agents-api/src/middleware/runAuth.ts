...
//   - App credential (X-Agent Fabric-App-Id): web_client (anon/asym JWT) or support_copilot
//
// Pluggable strategy chain (tried in order):
//   1. Bypass secret (AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET): dev/test override
...
function extractRequestData(c: { req: any }): RequestData {
  ...
  const agentId = c.req.header('x-agent-fabric-agent-id');
  const subAgentId = c.req.header('x-agent-fabric-sub-agent-id');
  const runAsUserId = c.req.header('x-agent-fabric-run-as-user-id');
  const appId = c.req.header('x-agent-fabric-app-id');
...
function buildExecutionContext(authResult: AuthResult, reqData: RequestData): BaseExecutionContext {
  ...
  if (authResult.metadata?.teamDelegation) {
    ...
    logger.warn(
      ...
      'API key agent scope mismatch: ignoring x-agent-fabric-agent-id header, using key-bound agent'
    );
  }
...
async function trySlackUserJwtAuth(token: string, reqData: RequestData): Promise<AuthAttempt> {
  ...
  if (!reqData.projectId || !reqData.agentId) {
    logger.warn(
      ...
      'Slack user JWT requires x-agent-fabric-project-id and x-agent-fabric-agent-id headers'
    );
    return {
      authResult: null,
      failureMessage:
        'Slack user token requires x-agent-fabric-project-id and x-agent-fabric-agent-id headers',
    };
  }
...
async function tryOAuthUserAuth(token: string, reqData: RequestData): Promise<AuthAttempt> {
  ...
  const tenantId =
    typeof payload['http://localhost/tenantId'] === 'string'
      ? (payload['http://localhost/tenantId'] as string)
      : undefined;
  ...
  if (!reqData.projectId || !reqData.agentId) {
    return {
      authResult: null,
      failureMessage:
        'OAuth user JWT requires x-agent-fabric-project-id and x-agent-fabric-agent-id headers',
    };
  }
...
function tryBypassAuth(apiKey: string, reqData: RequestData): AuthAttempt {
  if (!env.AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET) {
    return { authResult: null, failureMessage: 'no bypass secret configured' };
  }

  if (apiKey !== env.AGENT_FABRIC_AGENTS_RUN_API_BYPASS_SECRET) {
...
async function tryOAuthSupportCopilotAuth(
  ...
  const jwtTenantId = payload['http://localhost/tenantId'] as string | undefined;
...
/**
 * Authenticate using an app credential (X-Agent Fabric-App-Id header).
  ...
  if (app.type === 'web_client') {
    ...
    if (hasAuthConfigured) {
      ...
      if (asymResult.ok) {
        ...
        // `x-agent-fabric-challenge-solution` header is ignored on authenticated requests; abuse by an
    ...
    try {
      ...
      const { payload } = await jwtVerify(bearerToken, secret, { issuer: 'agent-fabric' });
...
async function runApiKeyAuthHandler(
  ...
  if (reqData.runAsUserId === 'system' || reqData.runAsUserId?.startsWith('apikey:')) {
    throw new HTTPException(400, {
      message: 'x-agent-fabric-run-as-user-id cannot be a system identifier',
    });
  }
