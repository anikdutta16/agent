import { checkProjectRepositoryAccess, getInstallationByGitHubId } from '@agent-fabric/agents-core';
...
app.post('/', async (c) => {
  ...
  if (!dbInstallation) {
    const errorMessage =
      'GitHub App installation not registered. Please connect your GitHub organization in the Agent Fabric dashboard.';
    logger.warn(
      { installationId: installation.installationId, repository: claims.repository },
