/**
 * AgentFabricCredentialProvider - Abstraction for Credential Management
 *
 ...
 * import { AgentFabricCredentialProvider } from '@agent-fabric/agents-sdk'
 *
...
/**
 * AgentFabricCredentialProvider - Unified credential management for Agent Fabric SDK
 *
 ...
 * const provider = new AgentFabricCredentialProvider()
 *
 ...
 * ```
 */
export class AgentFabricCredentialProvider implements CredentialStore {
  ...
  private createKeychainStore(config: KeychainCredentialConfig): CredentialStore {
    // Return a lazy-loading proxy that defers to @agent-fabric/agents-core when accessed
    ...
    return {
      ...
      get: async (key: string) => {
        const { createKeyChainStore } = await import('@agent-fabric/agents-core/credential-stores');
      ...
      checkAvailability: async () => {
        try {
          const { createKeyChainStore } = await import(
            '@agent-fabric/agents-core/credential-stores'
          );
          const store = createKeyChainStore(storeId);
...
/**
 * Factory function to create an AgentFabricCredentialProvider
 * @param config - Configuration options
 * @returns A new AgentFabricCredentialProvider instance
 */
export function createCredentialProvider(
  config: CredentialProviderConfig = DEFAULT_CONFIG
): AgentFabricCredentialProvider {
  return new AgentFabricCredentialProvider(config);
}
