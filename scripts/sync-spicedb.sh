...
#
# Environment:
#   - AGENT_FABRIC_AGENTS_RUN_DATABASE_URL (or DATABASE_URL) - Runtime database connection
...
#   AGENT_FABRIC_AGENTS_RUN_DATABASE_URL=<prod-db-url> \
...
# Use AGENT_FABRIC_AGENTS_RUN_DATABASE_URL (runtime DB has organization, member, project_metadata tables)
DATABASE_URL="${AGENT_FABRIC_AGENTS_RUN_DATABASE_URL:-${DATABASE_URL:-}}"
...
if [ -z "$DATABASE_URL" ]; then
  echo -e "${RED}❌ DATABASE_URL or AGENT_FABRIC_AGENTS_RUN_DATABASE_URL environment variable is required${NC}"
  echo ""
  echo "Example:"
  echo "  AGENT_FABRIC_AGENTS_RUN_DATABASE_URL=postgres://user:pass@localhost:5432/dbname ./scripts/sync-spicedb.sh"
  exit 1
fi
