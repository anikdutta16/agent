...
export async function configGetCommand(key?: string, options?: ConfigOptions): Promise<void> {
  ...
  const configPath =
    options?.config || options?.configFilePath || join(process.cwd(), 'agent-fabric.config.ts');

  if (!existsSync(configPath)) {
    console.error(chalk.red('No configuration file found.'));
    console.log(
      chalk.gray('Run "agent-fabric init" to create one, or specify a config file with --config')
    );
    process.exit(1);
  }
...
export async function configSetCommand(
  ...
  if (!existsSync(configPath)) {
    ...
    const configContent = `import { defineConfig } from '@agent-fabric/agents-cli';
