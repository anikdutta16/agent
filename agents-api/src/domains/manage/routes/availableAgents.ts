...
import { createProtectedRoute, inheritedAuth } from '@agent-fabric/agents-core/middleware';
...
async function tryTempTokenAuth(token: string): Promise<IdentifiedUser | null> {
  if (!env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY) {
    return null;
  }

  try {
    const publicKeyPem = Buffer.from(
      env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY,
      'base64'
    ).toString('utf-8');
