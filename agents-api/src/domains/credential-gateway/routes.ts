...
const TokenExchangeBodySchema = z.object({
  ...
  agentFabric_app_id: z.string().startsWith('app_'),
...
app.openapi(
  ...
  (async (c: any) => {
    ...
    const appRecord = await getAppById(runDbClient)(body.agentFabric_app_id);
    ...
    if (
      ...
      logger.info(
        { outcome: 'access_denied', userSub: sub, appId: body.agentFabric_app_id },
        'Token exchange denied'
      );
    ...
    if (!canUse) {
      logger.info(
        { outcome: 'project_denied', userSub: sub, appId: body.agentFabric_app_id },
        'Token exchange denied'
      );
      const err = rfc8693Error(403, 'access_denied', 'Access denied');
      return c.json(err.body, err.status);
    ...
    if (!supportCopilotConfig?.platform || !supportCopilotConfig?.credentialReferenceId) {
      logger.info(
        { outcome: 'app_not_configured', userSub: sub, appId: body.agentFabric_app_id },
        'Token exchange denied'
      );
      const err = rfc8693Error(403, 'access_denied', 'Access denied');
      return c.json(err.body, err.status);
    ...
    if (!resolvedRef) {
      logger.info(
        { outcome: 'project_branch_missing', userSub: sub, appId: body.agentFabric_app_id },
        'Token exchange denied'
      );
