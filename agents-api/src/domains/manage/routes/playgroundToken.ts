...
} from '@agent-fabric/agents-core/middleware';
...
app.openapi(
  ...
  async (c) => {
    ...
    if (!env.AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY) {
      throw createApiError({
    ...
    const privateKeyPem = Buffer.from(
      env.AGENT_FABRIC_AGENTS_TEMP_JWT_PRIVATE_KEY,
      'base64'
    ).toString('utf-8');
    const privateKey = await importPKCS8(privateKeyPem, 'RS256');

    // Derive kid from the corresponding public key (same logic as startup/playground-app.ts)
    const publicKeyPem = env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY
      ? Buffer.from(env.AGENT_FABRIC_AGENTS_TEMP_JWT_PUBLIC_KEY, 'base64').toString('utf-8')
    ...
    return c.json(
      {
        apiKey: token,
        expiresAt,
        appId: env.AGENT_FABRIC_PLAYGROUND_APP_ID || 'app_playground',
      },
      200
    );
  }
