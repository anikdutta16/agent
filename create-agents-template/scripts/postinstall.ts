...
// Opt-in guard: only export the Agent Fabric UI when a Vercel deploy of THIS template
// has explicitly opted in via CHAT_TO_EDIT-style AGENT_FABRIC_QUICKSTART_EXPORT=1.
...
// apps/mcp), which tries to run the `agent-fabric` CLI (may not be installed in
// a devDep-skipping production install) and crashes the build.
// Mirrors the fix shipped in PR #137 for private/chat-to-edit.
const shouldExport = process.env.AGENT_FABRIC_QUICKSTART_EXPORT === '1';
const skip = process.env.SKIP_APP_INSTALL === '1';

if (isVercel && shouldExport && !skip) {
  // Export the UI
  execSync('agent-fabric dev --export --output-dir ./apps/manage-ui', {
    stdio: 'inherit',
  });
