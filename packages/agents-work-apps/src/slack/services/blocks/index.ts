    ...
    .blocks(
      Blocks.Section().text(
        Md.bold('Already linked') +
          '\n\nYour Slack account is connected to Agent Fabric.\n\n' +
          Md.bold('Account:') +
          ` ${email}\n` +
          Md.bold('Linked:') +
          ` ${new Date(linkedAt).toLocaleDateString()}\n\n` +
          'To switch accounts, first run `/agent-fabric unlink`'
      ),
    ...
    .blocks(
      Blocks.Section().text(
        Md.bold('Account unlinked') +
          '\n\nYour Slack account has been disconnected from Agent Fabric.\n\n' +
          'Run `/agent-fabric link` to connect a new account.'
      )
    )
    .buildToObject();
}
...
export function createNotLinkedMessage() {
  return Message()
    .blocks(
      Blocks.Section().text(
        Md.bold('Not linked') +
          '\n\nYour Slack account is not connected to Agent Fabric. Run `/agent-fabric link` to connect.'
      )
    )
    .buildToObject();
}
...
export function createStatusMessage(
  ...
  const lines = [
    Md.bold('Connected to Agent Fabric'),
    '',
...
export function createSmartLinkMessage(linkUrl: string) {
  return Message()
    .blocks(
      Blocks.Section().text("To get started, let's connect your Agent Fabric account with Slack."),
      Blocks.Actions().elements(
...
export function createCreateAgentFabricAccountMessage(acceptUrl: string, expiresInMinutes: number) {
  return Message()
    .blocks(
      Blocks.Section().text(
        `${Md.bold('Create your Agent Fabric account')}\n\n` +
          "You've been invited to join Agent Fabric. Create an account to start using Agent Fabric agents in Slack."
      ),
      Blocks.Actions().elements(
