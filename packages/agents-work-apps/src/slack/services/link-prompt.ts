...
import { createCreateAgentFabricAccountMessage, createSmartLinkMessage } from './blocks';
...
export function buildLinkPromptMessage(result: LinkPromptResult) {
  if (result.type === 'auto_invite') {
    return createCreateAgentFabricAccountMessage(result.url, result.expiresInMinutes);
  }
  ...
  try {
    ...
    if (existingUser) {
      logger.debug(
        ...
        'User already has Agent Fabric account, skipping auto-invite'
      );
      return null;
    }
    ...
    logger.info(
      ...
      'Invitation created for Slack user without Agent Fabric account'
    );
