...
function ensureCopilotKeys() {
  ...
  const hasKey =
    envContent.includes('AGENT_FABRIC_COPILOT_JWT_PRIVATE_KEY=') &&
    !envContent.includes('# AGENT_FABRIC_COPILOT_JWT_PRIVATE_KEY=') &&
    !!envContent.match(/AGENT_FABRIC_COPILOT_JWT_PRIVATE_KEY=(.+)/)?.[1]?.trim();
  ...
  const vars = [
    { name: 'AGENT_FABRIC_COPILOT_JWT_PRIVATE_KEY', value: privateKeyBase64 },
    { name: 'AGENT_FABRIC_COPILOT_JWT_KID', value: kid },
    { name: 'PUBLIC_AGENT_FABRIC_COPILOT_APP_ID', value: 'app_copilot' },
  ];
...
async function ensureCopilotApp(apiUrl) {
  const appId = process.env.PUBLIC_AGENT_FABRIC_COPILOT_APP_ID;
  const privateKeyB64 = process.env.AGENT_FABRIC_COPILOT_JWT_PRIVATE_KEY;
  const kid = process.env.AGENT_FABRIC_COPILOT_JWT_KID;
  const bypassSecret = process.env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET;
    ...
    if (res.ok) {
      ...
      if (createdId && createdId !== appId) {
        ...
        const updated = envContent.replace(
          /PUBLIC_AGENT_FABRIC_COPILOT_APP_ID=.*/,
          `PUBLIC_AGENT_FABRIC_COPILOT_APP_ID=${createdId}`
        );
        writeFileSync('.env', updated);
        process.env.PUBLIC_AGENT_FABRIC_COPILOT_APP_ID = createdId;
...
if (isolatedName) {
  ...
  const p = state.ports;
  process.env.AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL = `postgresql://appuser:password@localhost:${p.doltgres}/agent_fabric`;
  process.env.AGENT_FABRIC_AGENTS_RUN_DATABASE_URL = `postgresql://appuser:password@localhost:${p.postgres}/agent_fabric`;
  ...
  process.env.AGENT_FABRIC_AGENTS_API_URL = apiUrl;
  process.env.PUBLIC_AGENT_FABRIC_AGENTS_API_URL = apiUrl;
  process.env.NEXT_PUBLIC_AGENT_FABRIC_AGENTS_API_URL = apiUrl;
  if (p.manage_ui) {
  ...
  await runSetup({
    ...
    pushProject: skipPush
      ...
      : {
          ...
          configPath: 'agents-cookbook/template-projects/agent-fabric.config.ts',
          apiKey: process.env.AGENT_FABRIC_AGENTS_MANAGE_API_BYPASS_SECRET,
          apiUrl,
        },
    devApiCommand: 'pnpm turbo dev --filter @agent-fabric/agents-api',
