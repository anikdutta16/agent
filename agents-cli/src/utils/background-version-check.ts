  ...
  checkForUpdate()
    .then((versionInfo) => {
      if (versionInfo.needsUpdate) {
        ...
        console.log(
          chalk.yellow(
            `\n⚠️  A new version of @agent-fabric/agents-cli is available: ${versionInfo.latest} (current: ${versionInfo.current})`
          )
        );
        console.log(chalk.gray('   Run `agent-fabric update` to upgrade to the latest version\n'));
      }
    })
    .catch(() => {
