...
// 3. ~/.agent-fabric/config (user global config)
//
// NOTE: We load these for secrets, but the CLI will IGNORE the URL configuration
// values (AGENT_FABRIC_AGENTS_MANAGE_API_URL, AGENT_FABRIC_AGENTS_RUN_API_URL) from .env files.
// URL configuration should only come from agent-fabric.config.ts or CLI flags.
