 ...
 *
 * Profiles are stored in ~/.agent-fabric/profiles.yaml and allow users to switch
 * between different Agent Fabric deployments (cloud, local, self-hosted).
 */
export class ProfileManager {
  ...
  constructor(options?: { profilesDir?: string }) {
    this.profilesDir = options?.profilesDir ?? join(homedir(), '.agent-fabric');
    this.profilesPath = join(this.profilesDir, 'profiles.yaml');
  }

  /**
   * Get the path to the profiles.yaml file
   * Creates the ~/.agent-fabric directory if it doesn't exist
   */
  ...
  saveProfiles(profiles: ProfilesConfig): void {
    ...
    const profilePath = this.getProfilePath();
    const tempPath = join(tmpdir(), `agent-fabric-profiles-${Date.now()}.yaml`);
  ...
  async checkCredentialExists(credentialRef: string): Promise<boolean> {
    try {
      const { KeyChainStore } = await import('@agent-fabric/agents-core/credential-stores');
      const store = new KeyChainStore('auth', 'agent-fabric-cli');
