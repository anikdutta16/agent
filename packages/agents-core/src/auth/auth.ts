...
export function createAuth(config: BetterAuthConfig): AuthInstance {
  ...
  const instance = betterAuth({
    ...
    plugins: [
      ...
      jwt(),
      oauthProvider({
        loginPage: `${env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL || 'http://localhost:3000'}/login`,
        consentPage: `${env.AGENT_FABRIC_AGENTS_MANAGE_UI_URL || 'http://localhost:3000'}/consent`,
        ...
        customAccessTokenClaims: async ({ user }) => {
          ...
          return {
            'http://localhost/tenantId': org?.id ?? undefined,
            'http://localhost/email': user.email,
          };
        },
      }),
      lastLoginMethod({
