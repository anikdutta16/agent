 ...
 *
 * Prerequisites:
 *   - AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL must be set.
...
const MANAGE_DB_URL = process.env.AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL;

const DRY_RUN = !process.argv.includes('--apply');

async function main() {
  if (!MANAGE_DB_URL) {
    console.error('AGENT_FABRIC_AGENTS_MANAGE_DATABASE_URL is not set. Export your .env first:');
      ...
      try {
        ...
        if (DRY_RUN) {
          ...
          await connection.query(
            `SELECT DOLT_COMMIT('-a', '-m', 'Drop deprecated scheduled_triggers and scheduled_workflows (migrated to runtime)', '--author', 'migration-script <migration@localhost>')`
          );
