...
SDK for building and managing agents in the Agent Fabric.

## Installation

```bash
npm install @agent-fabric/agents-sdk
```

## Usage

```typescript
import { agent, subAgent, tool } from '@agent-fabric/agents-sdk';
...
The SDK provides a clean abstraction for credential management through `AgentFabricCredentialProvider`:

```typescript
import { AgentFabricCredentialProvider, createCredentialProvider } from '@agent-fabric/agents-sdk';

// Simple memory-based storage (default)
const credentials = new AgentFabricCredentialProvider();
...
```typescript
import {
  AgentFabricTelemetryProvider,
  createConsoleTelemetryProvider,
  createOpenTelemetryProvider,
} from '@agent-fabric/agents-sdk';

// No-op (default - no telemetry overhead)
const telemetry = new AgentFabricTelemetryProvider();
...
```typescript
import { trigger, credentialReference } from '@agent-fabric/agents-sdk';
...
- `AgentFabricCredentialProvider` - Main credential provider class
- `createCredentialProvider()` - Factory function for creating providers

### Telemetry

- `AgentFabricTelemetryProvider` - Main telemetry provider class
...
This section documents how SDK versions relate to Agent Fabric Cloud runtime versions.
...
npm install @agent-fabric/agents-sdk @nangohq/node @nangohq/types

# With OpenTelemetry
npm install @agent-fabric/agents-sdk @opentelemetry/api @opentelemetry/sdk-node
```
