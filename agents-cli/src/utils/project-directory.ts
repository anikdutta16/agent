...
/**
 * Find project directory by looking for agent-fabric.config.ts
...
export async function findProjectDirectory(
  ...
  if (projectId) {
    ...
    if (projectId.includes('/') || projectId.includes('\\')) {
      ...
      if (existsSync(join(projectPath, 'agent-fabric.config.ts'))) {
        return projectPath;
      }
    } else {
  ...
  // Use find-up to look for agent-fabric.config.ts starting from current directory
  const foundConfigPath = await findUp('agent-fabric.config.ts');
