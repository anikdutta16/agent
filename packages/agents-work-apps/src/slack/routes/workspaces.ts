 ...
 * - Workspace settings (PUT): Agent Fabric org admin/owner only (requireWorkspaceAdmin middleware)
 * - Channel settings (PUT/DELETE): Agent Fabric org admin/owner OR channel member (requireChannelMemberOrAdmin middleware)
 * - Workspace uninstall (DELETE): Agent Fabric org admin/owner only (requireWorkspaceAdmin middleware)
 */
...
app.openapi(
  createProtectedRoute({
    ...
    description:
      'Uninstall the Agent Fabric Slack app from a workspace. The path parameter must be the raw Slack team ID (e.g. T012AB3C4).',
...
app.openapi(
  createProtectedRoute({
    ...
    description: 'List all users linked to Agent Fabric in this workspace',
...
app.openapi(
  ...
  async (c) => {
    ...
    try {
      ...
      const testMessage =
        message ||
        '*Test message from Agent Fabric*\n\nYour Slack integration is working correctly.';
        ...
        blocks: [
          ...
          },
          {
            ...
            elements: [
              {
                type: 'mrkdwn',
                text: '_This is a test message from the Agent Fabric dashboard_',
              },
            ],
          },
        ],
